...
**`ForwardedHeaders` indispensable pour que ce rate limiting (et celui de `RequestMagicLink`) fonctionne réellement** (2026-09-13) — en prod, l'API n'a aucun port publié sur l'hôte (`docker-compose.prod.yml`), Caddy est l'unique point d'entrée : sans `app.UseForwardedHeaders()`, `Connection.RemoteIpAddress` vaut toujours l'IP interne du conteneur Caddy pour toutes les requêtes, ce qui réduit les deux rate limiters à un seul compartiment partagé par tout le trafic externe — un attaquant peut l'épuiser pour bloquer l'admin légitime (DoS trivial), l'inverse de l'effet recherché. Fix : `builder.Services.Configure<ForwardedHeadersOptions>(...)` avec `KnownIPNetworks`/`KnownProxies` **vidés** (l'IP de Caddy sur le réseau Docker partagé n'est pas figée — approche recommandée par Microsoft pour un reverse proxy conteneurisé à IP non fixe) + `app.UseForwardedHeaders()` en tout premier middleware du pipeline (avant `UseCors`/`UseRateLimiter`/tout le reste). Sûr uniquement parce que l'API n'est jamais atteignable directement en prod ; en local (`docker-compose.yml`, port 5171 publié sans proxy devant), l'en-tête `X-Forwarded-For` est de facto non fiable — impact mineur limité à un bypass possible du rate limit sur le poste du développeur.
