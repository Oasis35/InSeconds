...
- Connection string : priorité `NF_INSECONDS_DB_POSTGRES_URI` (format legacy, converti via `BuildNpgsqlConnectionString` — non utilisé actuellement), fallback `ConnectionStrings:DefaultConnection` — **c'est cette branche qui est active** (`ConnectionStrings__DefaultConnection` posé par `docker-compose.prod.yml`, pointant vers le Postgres partagé `deploy/infra/`, cf. CLAUDE.md racine § Déploiement VPS).
