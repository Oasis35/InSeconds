...
Refonte 2026-09-16 : plus de mot de passe, plus de token dédié. L'auth admin est un **rôle sur le compte joueur** (`Player.IsAdmin`, cf. Domain plus bas) — un admin se connecte exactement comme un joueur normal (magic link, `/login`), avec le cookie `authToken` habituel. Endpoint minimal, pas de Command/Handler :
...
**Bypass Testing** : `Authorization: Bearer admin-token` en environnement `Testing` force `IsAdmin=true` — déplacé dans `PlayerAuthMiddleware` (double garde `IsEnvironment("Testing")` + constante fixe, jamais atteignable en Dev/Production), ce qui évite à tous les helpers de tests d'intégration (`IntegrationTestFactory`, `AdminTests.cs`, etc.) et fixtures E2E (`e2e/fixtures/api-client.ts`, `e2e/pages/admin.page.ts`) de changer. `POST /api/e2e/login-as-admin` (Testing-only, `Features/E2E/ResetEndpoint.cs`) crée/réutilise un joueur lié `IsAdmin=true` et pose son vrai cookie — utilisé par `AdminPage.login()` en E2E pour tester le vrai flux cookie plutôt que le seul bypass Bearer.

**Ancien mécanisme (avant 2026-09-16, historique)** : mot de passe partagé (`AdminPassword`/`ADMIN_PASSWORD`) → token Bearer aléatoire émis par `AdminTokenStore` (singleton en mémoire, 12h, perdu à chaque redéploiement) → `localStorage` côté front, avec rate limiting `admin-login` dédié et vérification `OriginValidator` en plus du token. Entièrement supprimé — cf. CLAUDE.md racine piège 5.
...
- **`PlayerAuthMiddleware`** : exécuté sur toutes les requêtes **sauf `/health`** (`/api/admin` n'est plus exclu depuis la refonte profils admin, 2026-09-16 — les routes admin sont désormais des endpoints joueur authentifiés comme les autres). Appelle `TryResolvePlayerAsync` (jamais `ResolveOrCreatePlayerAsync`) — **création paresseuse du Player** (2026-08-21) : un simple chargement de page (`/api/settings`, autocomplete Deezer, `/api/stats/today`...) ne crée plus de ligne en base, seul un vrai démarrage de partie ou une vraie connexion le fait. Stocke `PlayerId` **et `IsAdmin`** (`PlayerAuthResolution`, cf. `ICookieAuthService` ci-dessus) dans `httpContext.Items` **seulement si résolu** (sinon les deux clés sont absentes). Effet de bord bénin accepté : `LastSeenAt` est aussi mis à jour sur simple visite d'une route admin. **Bypass Testing** : si `env.IsEnvironment("Testing")` et header `Authorization: Bearer admin-token`, pose `IsAdminKey=true` inconditionnellement (double garde, jamais atteignable hors Testing) — remplace l'ancien bypass qui vivait dans `LoginEndpoint`.
- **`PlayerHttpContextExtensions`** : `GetPlayerId` throw si absent — n'est plus appelé par aucun endpoint depuis le passage à la création paresseuse (gardé/testé comme utilitaire générique pour un futur endpoint qui voudrait vraiment garantir la présence). `GetPlayerIdOrNull` retourne `null` sans throw : utilisé par **tous** les endpoints joueur hors `StartSession`/`GetCurrentPlayer`/`VerifyMagicLink`/`DevLogin`, y compris `SubmitAnswer`/`AbandonSession`/`UpdateListening` (répondent **404** si `null` — un visiteur sans cookie n'a par construction aucune session) et `Stats/Today` (répond un état "pas encore joué"). **`GetPlayerIsAdmin`** (2026-09-16) : `ctx.Items.TryGetValue(IsAdminKey, ...) && value is true` — utilisé par tous les endpoints Admin pour l'auto-protection (cf. `Admin/CheckAdminAuth` plus haut).
