...
Authentification **volontairement minimale** : `LoginEndpoint` invoque `bus.InvokeAsync<IResult>(new LoginCommand(password))`, `LoginHandler.Handle` compare le password à `IConfiguration["AdminPassword"]` en **temps constant** (`CryptographicOperations.FixedTimeEquals` sur les octets UTF-8, pas un simple `!=` sur `string` — évite une attaque temporelle caractère par caractère). Succès → `{token}` émis par `IAdminTokenStore.IssueToken()` (cf. Common/Auth ci-dessous — **plus une constante statique partagée**, un jeton aléatoire propre à chaque connexion). `IsAdminAuthenticated(HttpContext)` : vérifie `Authorization: Bearer <token>` (validité déléguée à `IAdminTokenStore.IsValid`) **et** l'origine de la requête (`OriginValidator.IsTrustedOrigin` contre `Cors:AllowedOrigins`, cf. Common/Auth ci-dessous) — méthode statique utilisée par tous les endpoints Admin pour l'auto-protection, y compris `Features/Admin/SendTestEmail/`. Combine les échecs en un seul `401` (ne révèle pas laquelle des vérifications a échoué). **Piège pour les tests** : tout client qui n'envoie pas d'`Origin`/`Referer` (un `HttpClient`/`fetch()` non-navigateur) se fait rejeter — cf. piège 22 racine, `IntegrationTestFactory` et `e2e/fixtures/api-client.ts` posent l'en-tête manuellement. `POST /api/admin/logout` révoque désormais réellement le jeton (`IAdminTokenStore.Revoke`) — avant l'introduction du store, ce endpoint ne faisait rien côté serveur puisque le token statique n'avait rien à invalider.

**Rate limiting anti brute-force sur `POST /api/admin/login`** (2026-09-12) — seule protection contre un brute-force du mot de passe unique (pas de lockout de compte, pas de CAPTCHA). `Program.cs` enregistre une politique `admin-login` (`AddRateLimiter`, fenêtre glissante `SlidingWindowRateLimiterOptions` : 10 requêtes / 5 min, partitionnée par IP `Connection.RemoteIpAddress`), appliquée uniquement à la route `/api/admin/login` via `RequireRateLimiting`, dépassement → **429**. `LoginEndpoint.MapAdminLogin(enableRateLimiting: bool = true)` — **désactivée en `Testing`** (`app.MapAdminLogin(enableRateLimiting: !isTesting)` dans `Program.cs`) car la suite E2E admin (~21 tests) + intégration font une vingtaine d'appels de login réels sur une fenêtre courte, ce qui déclencherait de faux 429 sans rapport avec un vrai brute-force. Jamais désactivée en Dev/Production.

**`ForwardedHeaders` indispensable pour que ce rate limiting (et celui de `RequestMagicLink`) fonctionne réellement** (2026-09-13) — en prod, l'API n'a aucun port publié sur l'hôte (`docker-compose.prod.yml`), Caddy est l'unique point d'entrée : sans `app.UseForwardedHeaders()`, `Connection.RemoteIpAddress` vaut toujours l'IP interne du conteneur Caddy pour toutes les requêtes, ce qui réduit les deux rate limiters à un seul compartiment partagé par tout le trafic externe — un attaquant peut l'épuiser pour bloquer l'admin légitime (DoS trivial), l'inverse de l'effet recherché. `app.UseForwardedHeaders()` en tout premier middleware du pipeline (avant `UseCors`/`UseRateLimiter`/tout le reste). **`KnownIPNetworks` peuplé (`Common/Networking/TrustedProxyNetworks`), pas vidé** — cf. piège 27 racine : une première version (même jour) le vidait entièrement avec `ForwardLimit=1` implicite, ce qui ne déroulait qu'un seul saut de `X-Forwarded-For` et faisait échouer `Connection.RemoteIpAddress` sur l'IP du edge Cloudflare (devant Caddy en prod, DNS proxied) au lieu de la vraie IP cliente — un edge Cloudflare dessert des milliers de visiteurs, tous compactés dans le même compartiment de rate limiting, ce qui a fait sauter le rate limiter `admin-login` en quelques minutes d'usage normal. Fix : `TrustedProxyNetworks.All` liste les plages privées RFC1918 (couvre Caddy quelle que soit l'IP Docker lui attribue — non falsifiable, la source TCP réelle est vérifiée par le noyau) **et** les plages publiques Cloudflare (permet de dérouler le saut suivant jusqu'à la vraie IP cliente), + `ForwardLimit=null` (sûr uniquement parce que `KnownIPNetworks` borne désormais la confiance, recommandation Microsoft). Un attaquant qui contournerait Cloudflare pour taper directement sur Caddy (UFW autorise 80/443 au monde entier) se retrouve avec sa vraie IP TCP hors de ces deux listes : le déroulement s'arrête à son IP réelle, non usurpable. En local (`docker-compose.yml`, port 5171 publié sans proxy devant), l'en-tête `X-Forwarded-For` est de facto non fiable (aucun des deux sauts Docker/Cloudflare n'existe) — impact mineur limité à un bypass possible du rate limit sur le poste du développeur.

**Jetons admin révocables** (2026-09-12) — le token `"admin-token"` était auparavant une constante statique partagée par tous les logins : `POST /api/admin/logout` ne faisait rien côté serveur (rien à invalider), et un token qui fuyait une fois (XSS, log, poste partagé) restait valide indéfiniment. `Common/Auth/AdminTokenStore` (`IAdminTokenStore`, singleton) émet désormais un jeton aléatoire par connexion (`MagicLinkTokenGenerator.GenerateRawToken()`, réutilisé tel quel), valide 12h, révocable via `Revoke` (appelé par `/api/admin/logout`). Stockage en mémoire (`ConcurrentDictionary`, process API unique sur le VPS) — un redéploiement invalide toutes les sessions admin en cours, compromis acceptable. **En `Testing`**, `IssueToken`/`IsValid`/`Revoke` retombent sur la constante fixe `"admin-token"` (jamais révoquée) car tous les tests d'intégration et fixtures E2E forgent directement `Authorization: Bearer admin-token` sans passer par un vrai login (`IntegrationTestFactory`, `e2e/fixtures/api-client.ts`, `e2e/pages/admin.page.ts`) — jamais ce comportement en Dev/Production.
