...
- **`GetTrackInfoAsync` → `DeezerTrackInfoProbe(bool Succeeded, DeezerTrackInfo? Info)`** : même distinction que `ProbePreviewAsync` (échec de requête vs vraie absence du track), réutilise `DefinitiveNoDataErrorCodes`. `DeezerTrackInfo(Artist, Title, PreviewUrl, DeezerTrackId, CoverHash, ReleaseYear?)`. `ReleaseYear` parsé depuis le champ Deezer `release_date` (format `yyyy-MM-dd`, `ExtractReleaseYear` — retourne `null` en silence si absent/format inattendu, jamais d'exception). `SearchTracksAsync` parse aussi `ReleaseYear` sur chaque résultat (même helper). Les appelants (`AddTrack`/`UpdateTrack`/`CreateChallenge`) distinguent `!probe.Succeeded` (503 `deezer_unavailable`, réessayer) de `probe.Info is null` (422 `invalid_track`, track réellement introuvable) — avant cette distinction, un simple hoquet Deezer (quota/busy) se traduisait par un faux 422 côté admin.
