...
```
GameComponent
 ├─ app-game-header          toujours affiché : playing/gélule série-gels/score/progression/avatar profil → (abandon) / (openStreak)
 ...
 ├─ toasts inline : streak invité (variante « Tu aurais gagné un gel ! ») [done | already_played],
 │   « série perdue » invité [welcome], « 1 gel a sauvé ta série » / « +1 gel gagné ! » connecté [done | already_played]
...
Règles produit : `docs/GAMEPLAY_RULES_FR.md` § Streak. État porté par `GameComponent` :
...
- **Anti-cheat « min écouté »** : `effect()` constructeur réagit à `chosenDuration()` — dès que le joueur s'engage sur un palier (`startPlay()`/`listenMore()`), `gameService.updateListening(sessionId, trackId, chosenDuration)` est appelé **immédiatement**, sans attendre que l'audio ait fini de jouer jusque-là (garde `sid>0 && tid>0 && dur>0`). Remplace depuis le 2026-09-19 l'ancien déclencheur sur `audio.state() === 'finished'` (cf. issue #153) : le back (`RequestHint/Handler.cs`) lit ce même champ (`GameSession.CurrentTrackMinListenedSeconds`) pour décider si un indice est débloqué, donc le faire dépendre de la fin de lecture retardait inutilement l'utilisation d'un indice déjà visible à l'écran.
- **Autocomplete + navigation clavier** (2026-09-22, extrait dans `AnswerSearchService`) — `searchQuery`/`artistAnswer`/`titleAnswer` (champs plats), `suggestions`/`showSuggestions`/`highlightedIndex` (signals), `query$` et la souscription `deezerSearch.search(query$)` vivent désormais dans **`services/answer-search.service.ts`** (cf. section dédiée plus bas). `BlindRoundComponent` n'en garde que des **alias directs** (`suggestions`/`showSuggestions`/`highlightedIndex`, mêmes signals) et un accesseur `get/set searchQuery()` délégant au service (pour garder `[(ngModel)]="searchQuery"` inchangé dans le template — **zéro changement de template**). `onQueryChange(q)`/`clearSearch(event)` restent des méthodes du **composant** : elles délèguent au service (`search.onQueryChange`/`search.clearAll`) **et** remettent `submission.showEmptyConfirm` à `false` — point de croisement entre les deux services (démarrer une nouvelle recherche ou vider le champ doit fermer la confirmation "réponse vide" éventuellement affichée). `onBlur`/`onSearchKeydown`/`selectSuggestion` sont des wrappers de délégation pure vers le service.
- **Navigation clavier dans la dropdown** (`AnswerSearchService.onSearchKeydown`, signal `highlightedIndex`, défaut `-1` = aucune sélection) : `↓`/`↑` déplacent la surbrillance (`moveHighlight`, cycle avec wrap-around ; depuis `-1`, `↓` va au premier élément et `↑` va directement au dernier), `Entrée` sélectionne l'élément en surbrillance s'il y en a une (`selectSuggestion`, sans soumettre le formulaire — comme un clic) sinon laisse le comportement par défaut (soumission du round), `Échap` ferme la dropdown sans toucher au champ. `highlightedIndex` est remis à `-1` à chaque nouvelle réponse de recherche, sélection, `clearSearch()` et `next()`. Le survol souris (`mouseenter` sur chaque `<li>`) resynchronise `highlightedIndex` pour garder clavier/souris cohérents. Couvert par `answer-search.service.spec.ts` + `blind-round.component.spec.ts` (12 tests, unitaire) et `e2e/specs/autocomplete-keyboard-nav.spec.ts` (4 tests, E2E).
- `listenMore()` : passe à `nextDuration()` et appelle `audio.extend(next)` — **prolongations libres et chaînables**, pas de limite au nombre d'appels (jusqu'au dernier palier configuré). Le template masque le bouton uniquement quand `nextDuration()` est `null` (dernier palier atteint) — il n'y a plus de garde `!audio.extended()` empêchant une deuxième prolongation. `nextDuration` est calculé sur `durations()` (liste déjà filtrée par l'anti-cheat de reprise), pas sur `settings.allowedDurations()` — évite une incohérence si l'anti-cheat retirait un jour un palier en milieu de liste (pas de bug actuel, la progression ne redescend jamais sous le seuil, mais robustesse par construction plutôt que par convention).
- **Soumission** (2026-09-22, résultat/toast extraits dans `AnswerSubmissionService`) — `result`/`showEmptyConfirm`/`isSubmitting`/`displayedScore`/`showNetworkError` (signals) et `networkErrorTimer` vivent désormais dans **`services/answer-submission.service.ts`** (cf. section dédiée plus bas). `BlindRoundComponent` n'en garde que des alias directs. `submit()`/`confirmSubmit()`/`doSubmit()` restent des méthodes du composant (elles émettent l'output `answered`, que seul un composant peut faire) : `submit()` appelle `search.resolveAnswer()` (split naïf sur `" - "` si aucune suggestion sélectionnée, en fallback texte libre — la logique vit désormais dans `AnswerSearchService`) ; si vide après trim → `submission.showEmptyConfirm.set(true)` (confirmation inline) plutôt qu'envoi direct. `skipNoPreview()`/`doSubmit()` partagent un helper privé `emitAnswer(overrides)` (évite de dupliquer `{trackId: this.track().id, ...}` à chaque émission).
- `setResult(r, isNetworkError=false)` (appelée par le parent via `viewChild`) : délègue à `AnswerSubmissionService.setResult(...)` (isSubmitting=false, animation `displayedScore` via `countUp` défaut 600ms, toast réseau 4000ms si `isNetworkError` — timer nettoyé/relancé proprement, cleanup automatique via `ngOnDestroy` du service). **Replay preview** reste dans le composant (a besoin de `track()`/`chosenDuration()`) : si preview existe et `chosenDuration()>0`, `audio.replayFull()` — rejoue le morceau en entier après révélation, indépendamment du palier choisi.
- **Indices (hints)** (2026-09-18, demande/révélation extraites dans `HintService` le 2026-09-22) — computeds `hint1Unlocked`/`hint2Unlocked` (`chosenDuration() >= settings.hintUnlockDurations()[n]`), utilisé directement dans le template (`@if (hint1Unlocked())`) pour que le bloc entier soit **masqué avant déblocage**, pas juste désactivé — `@if` Angular, pas de bouton présent dans le DOM (l'ancien alias `showAnyHintButton`, qui n'était qu'un pur alias de `hint1Unlocked()`, a été retiré le 2026-09-22) — ces computeds de déblocage par palier **restent dans `BlindRoundComponent`** (dépendent de `chosenDuration`, état de lecture du round, pas spécifique aux indices). Signaux `hint1Revealed`/`hint2Revealed`/`hintYear`/`hintArtistMasked`/`hintRequestPending` et méthodes `useHint1(sessionId, trackId)`/`useHint2(sessionId, trackId)` vivent désormais dans **`services/hint.service.ts`** (cf. section dédiée plus bas) — `BlindRoundComponent` n'en garde que des **alias directs** (mêmes noms, mêmes signals, template inchangé) et des méthodes `useHint1()`/`useHint2()` qui délèguent en passant `sessionId()`/`track().id`. Computeds `hint1Locked`/`hint2Locked` (= débloqué et pas encore révélé, pilotent l'affichage bouton pilule pointillée "Indice année"/"Indice artiste" vs pastille pleine une fois révélé) restent dans le composant, lisant les alias. **`useHint2` marque aussi `hint1Revealed=true`** (cumulatif : la réponse du niveau 2 contient déjà l'année, pas besoin d'un aller-retour séparé pour l'afficher). Le service expose `reset()`, appelé depuis `BlindRoundComponent.next()` (comme la remise à zéro de `chosenDuration`/`searchQuery`) — sinon un indice révélé sur un morceau resterait affiché sur le suivant. `resultHintUsed`/`resultHintPercent` (computeds sur `result()?.hintLevelUsed`/`hintPenaltyPercentApplied`) **restent dans le composant** (dérivés de `result()`, sans rapport avec le flux demande/révélation) — **le pourcentage vient directement de `SubmitAnswerResponse`**, jamais recalculé côté front à partir d'un setting (évite toute désynchronisation) ; conservés dans le `.ts` (couverts par un test dédié) mais **plus affichés** sur l'écran de révélation depuis le 2026-09-19 (badge "Indice utilisé (-X%)" retiré, cf. issue #153). Boutons/pastilles en violet (`--color-violet`) avec icône SVG ampoule (pas d'emoji) depuis la même date : bordure pointillée + fond transparent tant que non révélé, bordure continue + fond plus foncé une fois révélé. Détail complet des règles produit : `GAMEPLAY_RULES_FR.md` § Indices.
...
- `next()` : `audio.reset()`, `submission.reset()`, `search.reset()`, `chosenDuration.set(0)`, `hintService.reset()`, émet `nextTrack`.
- `ngOnDestroy` : `audio.reset()` uniquement — le nettoyage du timer réseau part avec `AnswerSubmissionService.ngOnDestroy()` (Angular appelle automatiquement `ngOnDestroy` des providers scopés au composant à sa destruction).
...
`@Injectable()` (pas root, scopé à **`BlindRoundComponent`** — extrait le 2026-09-22, premier service de la feature scopé un niveau plus bas que `GameComponent`). Signals `hint1Revealed`/`hint2Revealed`/`hintYear`/`hintArtistMasked`/`hintRequestPending`, méthodes `useHint1(sessionId, trackId)`/`useHint2(sessionId, trackId)` (appellent `GameFacadeService.requestHint(sessionId, trackId, level)`, no-op si déjà en cours ou déjà révélé) et `reset()`. Ne porte que le "quoi a été demandé/révélé" — le déblocage par palier (`hint1Unlocked` etc., dépendant de `chosenDuration`/`SettingsService`) et l'affichage du résultat soumis (`resultHintUsed`/`resultHintPercent`, dépendant de `result()`) restent dans `BlindRoundComponent`, qui n'expose que des alias directs de ces signals (pas de `computed()` autour, même instance) pour que le template n'ait rien à changer. Détail complet : section "Indices (hints)" plus haut.

## `services/answer-search.service.ts`

`@Injectable()` (pas root, scopé à **`BlindRoundComponent`**, extrait le 2026-09-22). Champs plats `searchQuery`/`artistAnswer`/`titleAnswer` (mutables directement, pas des signals — pour ne pas complexifier le binding `[(ngModel)]="searchQuery"` côté composant, qui expose un accesseur `get/set` délégant), signals `suggestions`/`showSuggestions`/`highlightedIndex`, `query$` + souscription à `DeezerAutocompleteService.search(query$)` dans le constructeur. Méthodes : `onQueryChange`/`onBlur`/`onSearchKeydown`/`selectSuggestion` (comportement inchangé, extrait tel quel de l'ancien `BlindRoundComponent`), `clearAll()` (utilisé par le bouton ✕ — vide aussi la dropdown), `resolveAnswer()` (split naïf sur `" - "` en fallback texte libre + trim, retourne `{artist, title}` avec `null` si vide — utilisé par `submit()`/`confirmSubmit()`), `reset()` (pour `next()` — **ne touche pas** `showSuggestions`, contrairement à `clearAll()`). Ne porte pas `showEmptyConfirm` (état de soumission, cf. `AnswerSubmissionService`) : `onQueryChange`/`clearSearch`, qui touchent aux deux, restent des méthodes de `BlindRoundComponent` qui orchestrent les deux services. Détail complet : section "le round de jeu" plus haut.

## `services/answer-submission.service.ts`

`@Injectable()` (pas root, scopé à **`BlindRoundComponent`**, extrait le 2026-09-22, `implements OnDestroy`). Signals `result`/`showEmptyConfirm`/`isSubmitting`/`displayedScore`/`showNetworkError`, `networkErrorTimer` privé. `setResult(r, isNetworkError=false)` : logique inchangée (isSubmitting=false, result.set, `countUp` sur `displayedScore`, toast réseau 4000ms si `isNetworkError`) — **sans** le replay audio (reste dans `BlindRoundComponent.setResult()`, qui appelle le service puis fait le replay, car il a besoin de `track()`/`chosenDuration()`). `reset()` : result/displayedScore/isSubmitting/showNetworkError + clear timer — **ne touche pas** `showEmptyConfirm` (parité avec l'ancien `next()`, qui ne le resettait pas non plus). `ngOnDestroy()` nettoie le timer réseau automatiquement à la destruction du composant (remplace l'ancien nettoyage manuel dans `BlindRoundComponent.ngOnDestroy`). Détail complet : section "le round de jeu" plus haut.
...
- **`game-header`** : présentationnel + injecte `PlayerSessionService` directement (pattern déjà établi côté admin pour des services `core/` injectés dans un composant présentationnel, cf. `admin/CLAUDE.md`). Inputs `required` : `playing`, `showStreak`, `streak` (`StreakDto | null`), `totalScore`, `currentIndex`, `trackCount` ; `pulse=false`. Outputs `abandon`, `openStreak`. **Gélule série/gels** (2026-09-23, bouton `data-testid="streak-pill"`, `data-mode`) à gauche : `pillMode` (`streakPillMode`) — `on` (flamme N | flocon N), `protected` (bordure cyan + `gel-pulse` ×3), `guest` (flamme N), `lost` (flamme grise + 0) ; `pillAnimation` pulse aussi la gélule `on` quand `pulse()` (gel gagné). Score en cours de partie : `left-4`. **Avatar profil** : **silhouette SVG** (plus l'initiale du pseudo), `title` « Mon profil — {pseudo} » (i18n `streakFreeze.avatarTitle`), dégradé `--gradient-primary`, `routerLink="/profile"`, visible si `playerSession.isLinked() && !playing()` (`right-2`). `game-header.component.spec.ts` couvre `pillMode`/`pillAnimation` + les outputs.
- **`game-footer`** : pas d'inputs/outputs. Injecte `LanguageService` (`currentLang = language.current` réexposé, `toggleLanguage()` bascule fr↔en) et `PlayerSessionService` **uniquement pour `isAdmin`** : le lien `/admin` n'est affiché que pour un compte admin. Liens `/privacy`, contact. **Icône de connexion retirée du footer** (2026-09) — l'accès à `/login`/`/profile` reste disponible via l'avatar du header (compte lié, cf. `game-header` ci-dessus) et les boutons outline de `welcome-screen`/`resume-screen` (guest) ; le footer n'injecte pas `Router`/`TranslateService`. Clés i18n `footer.login`/`footer.logout` supprimées (devenues orphelines). Testé (`game-footer.component.spec.ts`) : bascule fr→en/en→fr + persistance `localStorage` uniquement.

## `features/profile/` — écran Profil (2026-09)

Hors arbre `game/` (feature standalone sœur, route lazy `/profile` dans `app.routes.ts`) mais documenté ici car c'est la destination de l'avatar header ci-dessus, et il reprend la logique de déconnexion qui vivait auparavant dans `game-footer`.
...
- **Stats** : carte 2 colonnes streak (`playerSession.currentStreak`) / parties jouées (`playerSession.gamesPlayed`), toutes deux peuplées par `GET /api/players/me` (étendu 2026-09, cf. `src/back/InSeconds.Api/CLAUDE.md`). **Rangée « Gels »** (2026-09-23) pleine largeur : `<app-freeze-cells size="md">` + `freezesText` (« 2 gels · stock plein » / « 1 gel · prochain dans 5 jours »), depuis `playerSession.streak()`.
...
- **`welcome-screen`** : `trackCount` (required), `streak=null`, `hideLoginCta=false` → `startGame`. Compte connecté à série protégée (`isProtected`) : titre « Bon retour ! » + ligne cyan « Tes N jours de série au chaud grâce à un gel » (`data-testid="frozen-line"`). Injecte aussi `PlayerSessionService` directement (2026-09) : sous le bouton "Jouer", guest → bouton outline "Se connecter / Créer un compte" (`routerLink="/login"`) + texte d'accroche ; compte lié → lien discret "Connecté comme {{pseudo}}" (`routerLink="/profile"`).
- **`resume-screen`** : `completedCount`, `trackCount` (required), `abandonLoading=false` → `resumeGame`/`abandon`. Signal local `showAbandonConfirm` : double confirmation avant d'émettre réellement `abandon`. Texte d'avertissement selon le compte (2026-09-24) : invité → `resume.warningBody` (« série remise à zéro »), compte lié → `resume.warningBodyLinked` (« s'arrêtera, sauf si un gel la protège » — le jour manqué peut être couvert par un gel). Même choix pour la confirmation d'abandon en jeu de `GameComponent` (`abandonSheet.body` / `abandonSheet.bodyLinked`). Injecte `PlayerSessionService` (2026-09) : guest uniquement → bouton outline "Ne plus perdre mes parties" (`routerLink="/login"`) **sous** "Abandonner" (déplacé le 2026-09-23, maquette gel de série).
- **`status-screen`** : générique, réutilisé pour `no_challenge` ET `error` — `titleKey`/`bodyKey` (clés i18n passées par le parent) → `retry`.
- **`already-played-screen`** : `stats: TodayStatsResponse|null`, `abandoned=false`, `countdown` (required), `shareCopied`/`shareFailed=false` → `share`. Signal `showTrackDetails` (accordéon). Si `abandoned()` : message simple + countdown. Sinon : carte score/médiane (fallback `—` si `medianScore<=0`), `app-share-button`, puis l'accordéon délègue à **`<app-track-results-list [rows]="playedRows()">`** — `playedRows` (computed) mappe `stats().tracks` (`TrackStat`) → `TrackResultRow[]`. Lignes **identiques au récap** (chips `✓/✗`, durée, `+score` cliquable → pop-up histogramme). Plus de carte « Reviens sur n'importe quel appareil » sous la carte (retirée le 2026-09-23, seul le toast de série invité reste).
- **`final-recap-screen`** : **exporte `RoundResult`** — le contrat que `GameComponent` construit dans `onAnswered`/`resumePlaying` :
  ...
  `RoundResult` porte toujours `averageSecondsWhenCorrect`/`failureRatePercent`. Input `stats: TodayStatsResponse | null` (fourni par `GameComponent`, qui appelle `apiStatsToday()` à l'entrée de l'état `done`). L'accordéon délègue à **`<app-track-results-list [rows]="recapRows()">`** — `recapRows` (computed) mappe `results()` (`RoundResult`) + fusionne l'histogramme (`guessTimeDistribution`/`notFoundCount`) depuis `stats` par `position`. Résilience : la liste vient de `results()` (toujours présent) ; si `stats` est `null` la liste s'affiche quand même, seule la pop-up est indisponible. La pop-up (`openChart` signal, fermeture backdrop / ✕ / `Échap`) et le rendu des lignes vivent **dans `TrackResultsListComponent`** (`shared/track-results-list/`), plus dans cet écran. Plus de bannière « Garde ce score » sous la carte (retirée le 2026-09-23, alignement maquette gel de série — le toast de série invité suffit).
...
- **`shared/streak-sheet/`** (2026-09-23) : panneau bas série/gel. Inputs `streak` (`StreakDto`), `linked` (required). Outputs `closed`/`playNow`/`signup`. Variantes (`data-variant`) : `linked` (stock `<app-freeze-cells size="lg">`, puis **selon `atMax` (`freezes >= maxFreezes`, 2026-09-24)** : si pas encore au plafond, « Prochain gel à N jours · encore X », barre de progression ; si déjà au plafond, un message « Stock plein » à la place (`streakFreeze.sheet.full`) — `nextFreezeInDays` (back) reste un pur compteur de série qui ignore le stock, c'est ici que le front vérifie le plafond avant de l'afficher comme « tu vas gagner un gel », même garde-fou que `ProfileComponent.freezesText` pour la rangée « Gels » du profil ; puis la règle générique dans les deux cas), `protected` (frise `friezeDays` : ≤ 2 jours joués ✓ + jours manqués gelés + aujourd'hui, libellés `Intl` courts dans la langue courante ; « Jouer maintenant » / « Plus tard »), `guest` (« Créer un compte » / « Plus tard »). Fermeture fond (`<button>` accessible clavier) / `Échap`.
- **`shared/freeze-cells/`** (2026-09-23) : cases de stock de gels (`count`, `max`, `size` `sm|md|lg`, `fillLast` anime le remplissage de la dernière case pleine). Header toast « +1 gel gagné », panneau, profil.
- **`shared/streak-icon/`** (2026-09-23) : icônes SVG Lucide `flame`/`snowflake`/`heart-crack`/`user` (`currentColor`, `size` défaut `1.05em`).

## Services `core/` consommés (hors périmètre `game/` mais central ici)

- **`core/services/game.service.ts`** (`providedIn: 'root'`) : appels HTTP purs sur `/api/sessions` — `peekToday()` (`GET /today`, lecture seule, ne crée rien), `startToday()` (`POST`, crée session+cookie), `submitAnswer()` (`POST /answers`), `abandonSession()` (`PUT /abandon`), `updateListening()` (`PATCH /listening`), `requestHint()` (`POST /hint`, 2026-09-18).
...
- **`core/services/settings.service.ts`** : signals avec défauts codés en dur (utilisés tant que `/api/settings` n'a pas répondu) — `allowedDurations=[0.5,1,1.5,2,3,5,10]`, `guessTimerSeconds=20`, `tracksPerChallenge=5`, `durationScores={0.5:1000,1:850,1.5:700,2:550,3:400,5:250,10:100}`, `hintUnlockDurations=[5,10]` (2026-09-18, paliers de déblocage des 2 niveaux d'indice). `load()` fait un `catchError` + `console.warn` : **l'app démarre même si `/api/settings` échoue**.
...
- **`core/services/player-session.service.ts`** (`providedIn: 'root'`) : `playerId`/`isGuest`/`isLinked`/`email`/`pseudo`/`currentStreak`/`streak` (`StreakDto`, 2026-09-23)/`gamesPlayed`/`isAdmin` signals, peuplés par `load()` (`GET /api/players/me?peek=true`, jamais d'écriture). `updatePseudo(pseudo)` (2026-09) appelle `PUT /api/players/me/pseudo` et met à jour le signal `pseudo` local en cas de succès ; `requestEmailChange`/`confirmEmailChange`/`logout`/`ensureCreated` complètent. Consommé côté jeu par `GameComponent`, `game-header`, `game-footer` (`isAdmin`), `welcome-screen`, `resume-screen` (nudges connexion), et hors jeu par `features/profile/`, `features/admin/` et `shared/browser-id/`.
...
7. **`AnswerSearchService`/`AnswerSubmissionService` — croisement `showEmptyConfirm`** : `onQueryChange`/`clearSearch` doivent fermer la confirmation "réponse vide" (`submission.showEmptyConfirm.set(false)`) en plus de leur reset côté recherche — ce sont les deux seules méthodes qui orchestrent explicitement les deux services depuis `BlindRoundComponent` plutôt que de déléguer 1:1. Un futur refactor qui les rendrait purement délégantes réintroduirait le bug où retaper après un envoi vide laisse la confirmation affichée par-dessus la nouvelle saisie.
