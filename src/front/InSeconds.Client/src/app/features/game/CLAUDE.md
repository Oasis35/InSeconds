...
- **`shared/streak-sheet/`** (2026-09-23) : panneau bas série/gel. Inputs `streak` (`StreakDto`), `linked` (required). Outputs `closed`/`playNow`/`signup`. Variantes (`data-variant`) : `linked` (stock `<app-freeze-cells size="lg">`, « Prochain gel à N jours · encore X », barre de progression, règle), `protected` (frise `friezeDays` : ≤ 2 jours joués ✓ + jours manqués gelés + aujourd'hui, libellés `Intl` courts dans la langue courante ; « Jouer maintenant » / « Plus tard »), `guest` (« Créer un compte » / « Plus tard »). Fermeture fond (`<button>` accessible clavier) / `Échap`.
