...
- **Indices (hints)** (2026-09-18) — computeds `hint1Unlocked`/`hint2Unlocked` (`chosenDuration() >= settings.hintUnlockDurations()[n]`), `showAnyHintButton = hint1Unlocked()` (bloc entier **masqué avant déblocage**, pas juste désactivé — `@if` Angular, pas de bouton présent dans le DOM). Signaux `hint1Revealed`/`hint2Revealed`/`hintYear`/`hintArtistMasked`/`hintRequestPending`, computeds `hint1Locked`/`hint2Locked` (= débloqué et pas encore révélé, pilotent l'affichage bouton pilule pointillée "Indice année"/"Indice artiste" vs pastille pleine une fois révélé). `useHint1()`/`useHint2()` appellent `gameFacade.requestHint(sessionId, track().id, level)` — no-op si déjà en cours ou déjà révélé. **`useHint2` marque aussi `hint1Revealed=true`** (cumulatif : la réponse du niveau 2 contient déjà l'année, pas besoin d'un aller-retour séparé pour l'afficher). Tous ces signaux sont **réinitialisés dans `next()`** (comme `chosenDuration`/`searchQuery`) — sinon un indice révélé sur un morceau resterait affiché sur le suivant. `resultHintUsed`/`resultHintPercent` (computeds sur `result()?.hintLevelUsed`/`hintPenaltyPercentApplied`) — **le pourcentage vient directement de `SubmitAnswerResponse`**, jamais recalculé côté front à partir d'un setting (évite toute désynchronisation) ; conservés dans le `.ts` (couverts par un test dédié) mais **plus affichés** sur l'écran de révélation depuis le 2026-09-19 (badge "Indice utilisé (-X%)" retiré, cf. issue #153). Boutons/pastilles en violet (`--color-violet`) avec icône SVG ampoule (pas d'emoji) depuis la même date : bordure pointillée + fond transparent tant que non révélé, bordure continue + fond plus foncé une fois révélé. Détail complet des règles produit : `GAMEPLAY_RULES_FR.md` § Indices.
...
6. **Indices — signaux à réinitialiser dans `next()`** : `hint1Revealed`/`hint2Revealed`/`hintYear`/`hintArtistMasked`/`hintRequestPending` doivent être remis à zéro à chaque changement de morceau (déjà fait) — un futur refactor de `BlindRoundComponent` qui oublierait ce reset laisserait l'indice du morceau précédent affiché sur le suivant.
7. **Nudges de connexion = uniquement des rappels, jamais un blocage** — tous gardés par `PlayerSessionService.isLinked()` (`welcome-screen`/`resume-screen`/`already-played-screen`/`final-recap-screen`/toast de streak). Le jeu guest doit rester 100% fonctionnel sans jamais afficher ces éléments pour un compte lié — cf. décision d'architecture racine "pas de fermeture d'accès".
