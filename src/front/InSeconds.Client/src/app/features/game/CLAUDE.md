...
GameComponent
 ...
 └─ app-streak-sheet         [showStreakSheet] panneau série/gel → (closed) / (play) / (signup)
```
...
- `StreakSheetComponent` : `(play)` → `playFromStreakSheet()` (`beginGame()` depuis `welcome`, `beginResume()` depuis `resume_prompt`), `(signup)` → `router.navigate(['/login'])`.
...
- **`shared/streak-sheet/`** (2026-09-23) : panneau bas série/gel. Inputs `streak` (`StreakDto`), `linked` (required). Outputs `closed`/`play`/`signup`. Variantes (`data-variant`) : `linked` (stock `<app-freeze-cells size="lg">`, « Prochain gel à N jours · encore X », barre de progression, règle), `protected` (frise `friezeDays` : ≤ 2 jours joués ✓ + jours manqués gelés + aujourd'hui, libellés `Intl` courts dans la langue courante ; « Jouer maintenant » / « Plus tard »), `guest` (« Créer un compte » / « Plus tard »). Fermeture fond / `Échap`.
