...
Signals panneau de recherche/ajout (2026-09-16, remplace l'ancienne modale plein écran) : `addToPoolStatus: 'idle'|'loading'|'success'|'error'`, `addPanelOpen` (bandeau collapsible en haut de l'onglet Pool, ouvert/fermé sans jamais masquer le tableau), `addingTrackId: number|null` (DeezerTrackId en cours d'ajout — permet l'état loading/succès/erreur **par ligne** de résultat, plusieurs lignes pouvant être ajoutées à la suite sans fermer le panneau), `previewingUrl: string|null` (URL en cours d'écoute parmi les résultats, pour savoir quelle ligne afficher en ⏸), `searchLinked` (défaut `false` — indépendant : voir liaison des recherches ci-dessous).
...
- **`existingDeezerTrackIds`** — `Set<number>` des `deezerTrackId` déjà présents dans `allTracks()` (disponible ou utilisé). Alimente le badge d'avertissement "Déjà en pool" sur chaque résultat du panneau de recherche (`pool-search-panel`) — **non bloquant**, l'ajout reste possible même en cas de match.
...
**Panneau de recherche/ajout** (`pool-search-panel`, remplace `add-track-modal`) : `toggleAddPanel()` bascule `addPanelOpen` ; à la fermeture, `audioPreview.stop()` + reset `previewingUrl`/`poolSearchQuery`/`addingTrackId`/`addToPoolStatus`. `previewSearchResult(url)` — no-op si `url` absent, sinon `previewingUrl.set(url)` + `audioPreview.toggle(url)` (un seul lecteur audio partagé, comme pour la modale écoute). `addTrackFromPanel(track: DeezerTrackInfo)` — `addingTrackId.set(track.deezerTrackId)`, `addToPoolStatus.set('loading')`, `api.addTrack(track.deezerTrackId)` (plus de branche "update" : le bouton "↻ Actualiser" par ligne a été retiré, cf. `pool-tab` ci-dessous). Succès → `'success'` + `api.reloadPool()`, timer 2s retour `idle`/`addingTrackId=null` **sans fermer le panneau** (permet d'enchaîner plusieurs ajouts). Erreur → `'error'`, timer 3s même reset.
