...
- **`admin-login`** : `password` (propriété simple, `ngModel`), `loginStatus: 'idle'|'loading'|'error'|'rate_limited'`. `login()` → `api.login(password)`, catch distingue **429** (`HttpErrorResponse.status`, rate limiter `admin-login` côté back, cf. piège 27 CLAUDE.md racine — message `admin.tooManyAttempts`) de tout autre échec (401 réel, panne réseau... → `admin.wrongPassword`) — avant cette distinction (2026-09-13), un simple dépassement de quota s'affichait comme "mot de passe incorrect", ce qui a fait perdre du temps en prod à tort. Testé (`admin-login.component.spec.ts`) : succès, 401, 429, erreur non-HTTP.
