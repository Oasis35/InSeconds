...
**5 `rxResource`** — tous sauf `poolSearchResource` et `statsResource` sont **gardés pour le chargement paresseux par onglet** : `params` renvoie `undefined` (→ resource idle, aucun fetch) tant que `http.authenticated()` est faux **ou** que l'onglet concerné n'a pas été ouvert (`state.hasVisited(...)`, cf. `AdminStateService`).
