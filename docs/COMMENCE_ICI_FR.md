...
- **Modèle de données** : 10 tables (`Players`, `Tracks`, `DailyChallenges`, `DailyChallengeTracks`, `GameSessions`, `GameSessionAnswers`, `Settings`, `MagicLinkTokens`, `EmailChangeTokens`, `DataProtectionKeys`). Voir [`BACKEND_STRUCTURE_FR.md`](BACKEND_STRUCTURE_FR.md) pour le détail.
...
- Page admin (`/admin`) — login, pool (tableau unique + indicateur preview + panneau de recherche/ajout intégré avec lecteur), défis, stats dashboard, reset sessions
...
- `BackgroundService` refresh preview (à 23h UTC, avant la génération de minuit) — vérifie Deezer pour tous les tracks éligibles au défi du lendemain (jamais utilisés ou cooldown écoulé) par lots (rate-limit safe), met à jour `Track.HasPreview` uniquement sur réponse Deezer déterminée (jamais sur un échec quota/panne) ; relançable à la demande via `POST /api/admin/refresh-previews` (bouton « 🔄 Re-vérifier les previews » dans l'onglet Actions admin)
...
- Pool admin redesigné en tableau paginé (15 lignes/page) avec filtres combinables (texte, statut, preview), onglet "Actions" dédié (l'ancienne modale "↻ Actualiser" a été retirée le 2026-09-16 au profit du bouton « Re-vérifier les previews » de l'onglet Actions) — indicateur preview lu depuis `Track.HasPreview` en DB (stable, plus d'appel Deezer temps réel)
...
- **Chargement paresseux par onglet admin** (2026-08-29) — à l'ouverture de l'admin, seul `GET /api/admin/stats` (Dashboard, léger) part ; Pool (`/api/admin/tracks`) et Défis (`/api/admin/challenge-stats` + `/api/admin/challenges`) ne chargent qu'à l'ouverture de leur onglet (`AdminStateService.hasVisited`). La partie lourde « Stats par défi » a été **scindée de `GET /api/admin/stats`** vers un nouvel endpoint `GET /api/admin/challenge-stats`
- Tests E2E Playwright (86 scénarios : 62 jeu + 24 admin, CI GitHub Actions)
- Tests d'intégration backend (`InSeconds.Api.IntegrationTests`) — Testcontainers.PostgreSql + `WebApplicationFactory<Program>` + Respawn, 180 tests couvrant les comptes (magic link, changement d'email), le gel de série, les indices, StartSession, GetTodaySession (peek), SubmitAnswer, AbandonSession, Stats, AdminStats + ChallengeStats (`GET /api/admin/challenge-stats`, dont la liste des joueurs par défi), Players (`GET /api/players/me`), PlayerSoftDelete, SessionEdgeCases (dont UpdateListening et streak sur la date du défi), ChallengeGeneration, LazyChallengeGeneration (régénération à la volée + 503 si pool insuffisant), Admin/Tracks, Admin/Challenges, Admin/RefreshPreviews, Admin/Settings, DeezerSearch (nettoyage + déduplication autocomplete), HealthCheck (dont date de build), job CI dédié `integration-tests`
...
- **Refacto frontend** — `game.component` découpé en header + footer + 5 screens + `GameFacadeService` + `DeezerAutocompleteService` + services extraits (`AnswerSearchService`, `AnswerSubmissionService`, `HintService`, `GameShareService`, `LeaveConfirmationService`) (`features/game/services/`) ; `admin.component` en shell + 7 services (`AdminHttpService`, `AdminStateService`, `AdminApiService`, `AdminStatsService`, `AdminPoolService`, `AdminActionsService`, `PoolAudioPreviewService`) + 8 sous-composants ; palette CSS centralisée en variables `:root` ; `ShareButtonComponent` réutilisable
- **Changement d'email depuis `/profile`** (2026-09-14) — confirmation par email à la nouvelle adresse (token 15 min), flux calqué sur le login par magic link. Détail : `CLAUDE.md` racine, `src/back/InSeconds.Api/CLAUDE.md`
- **Tests unitaires frontend** — environ 360 tests Karma/Jasmine (`GameService`, `SettingsService`, `LanguageService`, `GameFooterComponent`, `AdminHttpService`, `AdminStateService` (onglet actif + `visitedTabs`), `AdminStatsService`, `AdminPoolService` avec l'autonomie du pool, `PoolAudioPreviewService` (lecteur audio partagé), `BlindRoundComponent` avec la navigation clavier de l'autocomplete, `GuessTimeChartComponent` + `TrackResultsListComponent`, `ClipboardService`, `BrowserIdComponent`, `ChallengesTabComponent` avec les chips d'identité joueur, `ProfileComponent`/`ConfirmEmailComponent`/`PlayerSessionService` avec le changement d'email) ; job CI `unit-tests-front` (`ChromeHeadless`)
- **Indicateur joueurs par défi + ID navigateur en admin** (2026-08-15) — `GET /api/players/me` expose le `PlayerId` du cookie courant (illisible côté front sinon : cookie `HttpOnly` + chiffré) ; la liste des joueurs par défi (`ChallengeStatsDto.Players`) est renvoyée par `GetChallengeStats` (à l'origine `GetAdminStats`, scindé le 2026-08-29) ; `ChallengesTabComponent` affiche un chip ID court par joueur (copie au clic, surbrillance auto « toi ») ; `BrowserIdComponent` (partagé) affiche l'ID + bouton copier sur l'écran de login et dans le shell admin ; `ClipboardService` mutualisé entre le partage de score (jeu) et ces deux nouveaux composants
...
- **`robots.txt` + `sitemap.xml`** (2026-09-18) — fichiers statiques dans `public/` du front (avant : fallback SPA renvoyant l'`index.html`) ; `robots.txt` exclut `/admin`/`/login`/`/profile`, `sitemap.xml` liste `/` et `/privacy`. Content-Type vérifié par le job CI `nginx-headers`. Détail : `CLAUDE.md` racine
- **Nettoyage des titres affichés au-delà de l'autocomplete** (2026-08-17) — `TextNormalizationHelpers.CleanDisplayTitle` (extrait de `SearchEndpoint.CleanTitle`) retire les parenthèses/crochets de tout titre de morceau révélé après coup : réponse par morceau + récap final (`SubmitAnswer`), reprise de partie (`StartSession`), écran "déjà joué" (`Stats/Today`), onglet Défis admin — sections Stats par défi (`GetChallengeStats`) et Historique (`GetChallenges`, corrigé après coup le même jour). `Track.Title` en base et la recherche admin restent bruts (nécessaires pour le re-sync Deezer)
...
- **Pas de timer de saisie** : `GuessTimerSeconds` (20 s) existe en base mais n'est appliqué nulle part
...
- Un `Player { IsGuest=true, Pseudo=null }` + son cookie + sa `GameSession` sont créés au moment où le joueur clique « Commencer à jouer » (`POST /api/sessions`) — **pas** au simple chargement de la page, qui ne fait qu'un peek `GET /api/sessions/today` lecture seule
...
**Coexistence avec les comptes utilisateurs** : le jeu guest reste ouvert à tous, sans aucune fermeture d'accès — un compte lié (login par magic link, historique/streak partagés entre appareils) est optionnel, pas une condition pour jouer. Un guest qui ne s'intéresse jamais aux comptes n'est pas affecté ; les rappels de connexion restent ponctuels (boutons discrets sur l'accueil et la reprise, toast de série) et le footer ne porte plus d'icône de connexion.
