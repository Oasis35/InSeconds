...
- Tests E2E Playwright (63 scénarios : 39 jeu + 24 admin, CI GitHub Actions)
- Tests d'intégration backend (`InSeconds.Api.IntegrationTests`) — Testcontainers.PostgreSql + `WebApplicationFactory<Program>` + Respawn, 114 tests couvrant StartSession, GetTodaySession (peek), SubmitAnswer, AbandonSession, Stats, AdminStats + ChallengeStats (`GET /api/admin/challenge-stats`, dont la liste des joueurs par défi), Players (`GET /api/players/me`), PlayerSoftDelete, SessionEdgeCases (dont UpdateListening et streak sur la date du défi), ChallengeGeneration, LazyChallengeGeneration (régénération à la volée + 503 si pool insuffisant), Admin/Tracks, Admin/Challenges, Admin/RefreshPreviews, Admin/Settings, DeezerSearch (nettoyage + déduplication autocomplete), HealthCheck (dont date de build), job CI dédié `integration-tests`
