...
- **`Track.ReleaseYear` absent** (pool pas encore backfillé, cf. `POST /api/admin/refresh-release-years`) : l'indice niveau 1 renvoie `null`, le front masque simplement la pastille année — pas d'erreur, l'indice niveau 2 (artiste) reste disponible indépendamment.
