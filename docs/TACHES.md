...
- [x] 9 tables (`Players`, `Tracks`, `DailyChallenges`, `DailyChallengeTracks`, `GameSessions`, `GameSessionAnswers`, `Settings`, `MagicLinkTokens`, `EmailChangeTokens`) + `DataProtectionKeys` (Data Protection) + configurations EF + migrations PostgreSQL
...
- [x] `SessionStatus` enum (Pending=0, Completed=1, Abandoned=2, Expired=3) — `Abandoned` = clic explicite sur « Abandonner », `Expired` = `Pending` basculé par l'expiry paresseuse ; stats/leaderboard filtrés sur `Completed`
...
- [x] Page admin : accès par rôle `Player.IsAdmin` sur le cookie joueur (magic link, cf. piège 5 du `CLAUDE.md`), pool morceaux, création défis, reset sessions
...
- [x] `GET /api/admin/stats` — dashboard admin : KPIs du jour, activité 30 jours, répartition joueurs (les stats par défi sont sorties dans `GET /api/admin/challenge-stats` le 2026-08-29, cf. plus bas)
- [x] Page admin — Pool : tableau paginé (15 lignes/page), filtres combinables (texte, statut, preview), indicateur preview (vert/rouge) via `TrackDto.HasPreview`, panneau de recherche/ajout intégré à l'onglet (a remplacé la popup le 2026-09-16, badge « Déjà en pool », bouton lien/délie avec le filtre) avec recherche Deezer + lecteur preview 30s
...
- [x] Actualisation d'un morceau sans preview (`PUT /api/admin/tracks/{id}`) — bouton "↻ Actualiser" ouvre la modale en mode update, écrase DeezerTrackId/Artist/Title/CoverHash — **bouton retiré du front le 2026-09-16** (remplacé par « Re-vérifier les previews » de l'onglet Actions), endpoint et tests d'intégration conservés
...
- [x] Seed enrichi : 5 morceaux sans preview (The Beatles, Pink Floyd, Bob Dylan, Led Zeppelin, Fleetwood Mac — IDs >= `9_000_000_000`) (le flux « ↻ Actualiser » a depuis été retiré du front)
...
- [x] `BlindRoundComponent` — lecture auto au premier palier (plus de choix initial depuis la refonte DA), prolongation, timer, feedback stats
- [x] `AdminComponent` — login, pool, défis, recherche Deezer
- [x] `playerAuthInterceptor` (`withCredentials` sur toutes les requêtes `/api`, admin compris — l'`adminAuthInterceptor` initial a été supprimé avec le passage à l'auth admin par rôle)
...
- [x] Polish UX blind round — loading sur "Valider" (`isSubmitting`), bouton `✕` d'effacement, score en count-up animé (le tooltip de points au survol des paliers a été retiré avec l'auto-play), toast d'erreur réseau (4s)
...
- [x] **Optimisations performance front** (2026-07-02) — `ChangeDetectionStrategy.OnPush` sur tous les composants Angular (35 au 2026-09-18), `takeUntilDestroyed(destroyRef)` sur toutes les subscriptions Observables (`game.component.ts`, `blind-round.component.ts`, `admin-pool.service.ts`, `admin-actions.service.ts`), tracking des handles `setTimeout` + `clearTimeout()` avant recréation dans `admin-pool.service.ts` et `admin-actions.service.ts`
...
- [x] **Indicateur joueurs par défi + ID navigateur en admin** (2026-08-15) — `ChallengesTabComponent` affiche un chip ID court par joueur sous chaque défi (clic = copie l'ID complet, surbrillance auto + libellé « toi » si l'ID correspond au navigateur courant) ; `BrowserIdComponent` (`shared/browser-id/`, sans `@Input`/`@Output`) affiche l'ID court du navigateur + bouton copier, monté une seule fois dans `admin.component.html` (visible sur l'écran de login et dans le shell admin) ; `PlayerIdentityService` (`core/`, root — depuis remplacé par `PlayerSessionService`) charge `GET /api/players/me` une seule fois par session ; `ClipboardService` (`core/`, root) mutualise `navigator.clipboard.writeText`, réutilisé par `game.component.copyToClipboard` (comportement de partage de score inchangé)
...
- [x] Anti-CSRF (`OriginValidator`) sur `VerifyMagicLink` uniquement — seul endpoint qui pose un cookie sur un `POST` (`ConfirmEmailChange` et les routes admin ne l'utilisent pas)
- [x] Connexion rapide dev (3 comptes seed, `Features/Auth/DevLogin/`, jamais en Testing/Production)
- [x] Front : `PlayerSessionService`, écrans `/login` + `/login/verify`, icône de connexion dans le footer, pseudo dans les chips joueur admin
...
- **Pas de Leaderboard permanent** — app volontairement simple, pas de classement inter-jours (les pseudos existent depuis les comptes optionnels, mais ne servent à aucun classement). Un classement anonyme du jour (top scores + médiane) reste envisageable
- **Pas d'inscription obligatoire** — le jeu reste 100 % jouable en guest ; les comptes sont optionnels (magic link, sans mot de passe) et servent uniquement à retrouver historique/streak sur plusieurs appareils
