...
- **`SubmitAnswerResponse` — histogramme** : en plus de `AverageSecondsWhenCorrect`/`FailureRatePercent`, la réponse porte `GuessTimeDistribution` (`DurationBucketDto(decimal DurationSeconds, int Count)[]` : un bucket par palier `AllowedDurationsSeconds`, comptes des réponses artiste-ou-titre correctes pour ce morceau, réponse courante incluse) et `NotFoundCount`. Requête agrégée dédiée `GroupBy(ListenedDurationSeconds)` sur les réponses correctes, lue avant l'insert puis combinée en mémoire (`IX_GameSessionAnswers_DailyChallengeTrackId` couvre). Consommé par `GuessTimeChartComponent` sur l'écran de révélation.
