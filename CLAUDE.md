...
Stack : .NET 10 / Wolverine / EF Core / PostgreSQL côté back, Angular 22 / Tailwind v4 / SCSS côté front, Docker Compose pour back + DB. **Hébergé sur un VPS OVH** (Debian 13, Gravelines) — front + API + PostgreSQL en conteneurs Docker derrière Caddy (reverse proxy, HTTPS auto), détail complet dans [Déploiement VPS](#déploiement-vps) plus bas. `environment.ts#appUrl` pointe vers `https://inseconds.cc`, le nom de domaine public. **La page de rebond GitHub Pages (`oasis35.github.io/InSeconds`) a été retirée** (remplacée par le domaine public, plus lisible et plus stable) — ne pas la recréer.
...
- `src/environments/environment{,.development}.ts` : `apiUrl` + `appUrl`, swap auto via `fileReplacements`. En prod `apiUrl = https://api.inseconds.cc` (A record Cloudflare Proxied vers l'IP du VPS, cert Let's Encrypt géré par Caddy via challenge DNS-01, cf. [Déploiement VPS](#déploiement-vps))
...
- **CORS** : le back autorise `http://localhost:5173`, `http://localhost:65075`, `https://p01--front--b5cnx77tvxgb.code.run` (ancienne URL, non utilisée) et `https://inseconds.cc` + `https://www.inseconds.cc` (nom de domaine public) dans `appsettings.json` (`Cors:AllowedOrigins`)
...
- `DecorBackgroundComponent` (`shared/decor-background/`) : décor DA (grille/scanlines) en arrière-plan
- `DeezerBadgeComponent` (`shared/deezer-badge.component.ts`, fichier plat sans sous-dossier) : badge officiel "À écouter sur Deezer"
- `GuessTimeChartComponent` (`shared/guess-time-chart/`) : histogramme réutilisable « en combien de temps les autres ont trouvé » — une barre par palier d'écoute (comptes de bonnes réponses) + une barre « ✗ » finale. Inputs `distribution` (`DurationBucketDto[]`), `notFoundCount`, `highlightDuration` (surbrillance de la colonne du joueur), `highlightNotFound`, `titleKey` (clé i18n du titre, masqué si vide), `showCounts` (défaut `false` — affiche le nombre de joueurs au-dessus de chaque barre, activé dans la vue admin). Présentationnel pur, couleurs via tokens `:root` (`--color-violet` barres, `--color-accent-3` surbrillance, `--color-fail` barre « ✗ »). Utilisé sur l'écran de révélation du blind round (`blind-round.component.html`), dans la pop-up de `TrackResultsListComponent`, et dans la pop-up de `ChallengesTabComponent` (onglet Défis admin, `showCounts=true`)
- `TrackResultsListComponent` (`shared/track-results-list/`) : liste accordéon des morceaux d'un défi (pochette, chips `✓/✗ Artiste / Titre`, durée écoutée, `% ratés`/moyenne, `+score`) **+ pop-up histogramme** (`GuessTimeChartComponent`, fermeture backdrop / ✕ / `Échap`) au clic sur le `+score` d'un morceau. Input unique `rows: TrackResultRow[]` (champs joueur `null` autorisés → chips/durée masqués ; `guessTimeDistribution` vide → score non cliquable). Mutualisé par `FinalRecapScreenComponent` (mappe `RoundResult` + histogramme depuis `stats`) et `AlreadyPlayedScreenComponent` (mappe `TrackStat`). Le contour (carte, bouton « Voir les morceaux / Masquer ») reste géré par chaque écran
...
| `GameSessions` | 1 entrée/joueur/jour (unique sur `PlayerId+DailyChallengeId`), `Status` (Pending=0/Completed=1/Abandoned=2/Expired=3), `CompletedAt`/`AbandonedAt` nullable, index leaderboard + index `(ChallengeId, Status)` |
...
| `AllowedEmails` | whitelist admin des emails autorisés à créer/utiliser un compte lié (`Email` normalisé lowercase, index unique, `CreatedAt`) |
| `MagicLinkTokens` | tokens de connexion à usage unique — seul `TokenHash` (SHA-256) est stocké, jamais le token brut ; `ExpiresAt` (15 min self-service / 7 jours invitation admin), `ConsumedAt?` |
...
- **Anti-rejeu** : contrainte unique `(PlayerId, DailyChallengeId)` sur `GameSessions`. `Completed`, `Abandoned` ou `Expired` → 409. `Pending` → reprise avec `IsResuming=true`
- **`SessionStatus`** : `Pending` = en cours (reprendre possible), `Completed` = terminée (N morceaux répondus), `Abandoned` = **clic explicite** sur « Abandonner », `Expired` = un `Pending` que l'expiry paresseuse (au prochain `StartSession`) a basculé — le joueur a quitté sans terminer et sans abandonner. Les stats admin distinguent les deux.
...
`pg_dump`/`pg_restore` via un conteneur `postgres:17-alpine` jetable (même version majeure que la prod, évite tout risque d'incompatibilité de format de dump) :

```bash
# Dump depuis une source externe (connection string à adapter)
docker run --rm postgres:17-alpine pg_dump --no-owner --no-privileges -Fc '<connection-string>' > dump.custom

# Restore dans le Postgres partagé du VPS
infra_pw=$(grep '^INSECONDS_DB_PASSWORD=' deploy/infra/.env | cut -d= -f2-)
docker run --rm -i --network shared-postgres -e PGPASSWORD="$infra_pw" postgres:17-alpine \
  pg_restore --no-owner --no-privileges --clean --if-exists -h shared-postgres -p 5432 -U inseconds -d inseconds < dump.custom
```

`--no-owner --no-privileges` évite les erreurs `ALTER OWNER`/`GRANT` si la source utilise un rôle qui n'existe pas côté VPS. Des erreurs sur des extensions de monitoring absentes (`pg_stat_statements`, etc.) sont bénignes — `pg_restore` continue et restaure le reste normalement (`errors ignored on restore: N`, pas un échec).
...
**SonarCloud** tourne en *Automatic Analysis* (pas de job dans `ci.yml`). ⚠️ **L'Automatic Analysis de ce projet n'ingère pas `sonar-project.properties`** (testé : sans effet). Les exclusions se posent **dans l'UI SonarCloud** : `Administration → Analysis Scope → Source File Exclusions`. Exclusions en place :
- `**/api.generated.ts` — le client NSwag généré produit une paire de méthodes request/process au boilerplate quasi identique par endpoint, que la détection de copier-coller comptait comme « duplication sur le nouveau code » à chaque nouvel endpoint (échec Quality Gate, seuil 3 %).
- `**/Common/Email/Templates/**` — l'HTML des emails transactionnels (`_layout.html` + fragments, cf. `src/back/InSeconds.Api/CLAUDE.md` § Auth). Sonar y appliquait ses règles HTML *web* (`table` de layout, attributs `bgcolor`/`cellpadding`/`width`/`align` « dépréciés ») → Maintainability B, alors que layout-table + ces attributs sont obligatoires pour un rendu fiable sous Outlook (moteur Word : pas de fl/grid, CSS externe ignoré). Les styles inline répétés d'un template à l'autre gonflaient aussi la duplication. Non pertinent à gater — même logique que `api.generated.ts`.

Le fichier `sonar-project.properties` racine est conservé uniquement comme documentation d'intention (et serait honoré si on passait un jour à une analyse par scanner en CI).
...
- **Job `e2e`** : tests Playwright E2E (dépend de `back` + `unit-tests` + `unit-tests-front` + `front`). Tourne sur `ubuntu-latest` avec un service PostgreSQL (base `inseconds_e2e`). Lance le back en mode Testing sur le port 5171 avec `--no-launch-profile`, attend que `/api/settings` réponde, démarre Angular avec `ng serve --configuration e2e-ci`, puis exécute `npx playwright test`. Upload le rapport HTML en artifact **toujours** (`if: always()`, pas seulement en cas d'échec).
- **Job `nginx-headers`** (dépend de `front`) : seul job qui construit et sert réellement l'image Docker de prod (`Dockerfile.prod`, nginx). Exécute `scripts/check-nginx-cache-headers.sh` — build l'image, lance le conteneur, vérifie via `curl` que `Cache-Control` est `no-cache` sur `/`, `/i18n/*.json` et les routes SPA (`/admin`), et `public, max-age=31536000, immutable` sur les bundles JS hashés. Aucun autre job ne teste `nginx.conf` (les tests E2E tournent contre `ng serve`, pas contre l'image Docker) — cf. piège 20.
...
17. **Clés Data Protection : persistées en base — ne pas retirer** (`PersistKeysToDbContext<ApplicationDbContext>()`, table `DataProtectionKeys`, migration `PersistDataProtectionKeys`). Sans ça, les clés vivent dans le filesystem du conteneur et sautent à chaque redéploiement **ou redémarrage** : le cookie `authToken` (chiffré avec ces clés) devient illisible → nouveau guest créé → streak et historique perdus pour tous les joueurs (bug historique, corrigé 2026-07-14). Note : les cookies émis *avant* le premier déploiement de ce fix seront invalidés une dernière fois (anciennes clés perdues avec le conteneur).
18. **Streak : toujours comparer sur `DailyChallenge.Date`, jamais sur la date de complétion** — l'ancien code de `SubmitAnswer/Handler.cs` comparait `LastPlayedDate` à `DateTime.UtcNow` au moment où le joueur *termine* : terminer le défi de lundi mardi à 00:15 UTC cassait la streak. Corrigé (2026-07-11) : la streak s'incrémente si `LastPlayedDate == DailyChallenge.Date - 1 jour`, et `LastPlayedDate` stocke la date du défi. Couvert par les tests d'intégration `Streak_DefiDeLaVeilleCompleteApresMinuit_ContinueLaStreak` et `Streak_JourManque_RemetLaStreakA1`. Ne pas réintroduire de `DateTime.UtcNow` dans cette logique.
19. **`Task.Delay(~24h)` peut se réveiller juste avant minuit (horloge murale)** — `Task.Delay` mesure du temps *monotone* : sur 24h, la dérive avec l'horloge murale (ajustements NTP) peut le réveiller une fraction de seconde avant l'heure visée. Incident du 2026-07-13 : le générateur s'est réveillé alors que `DateTime.UtcNow` donnait encore le 12/07 → « défi déjà présent » → replanifié au minuit suivant → **journée du 13/07 sans défi** (pas de retry, `AlreadyExists` = succès). Fix : les jobs nocturnes attendent une **cible d'horloge murale** via `DailySchedule.DelayUntilAsync(target)` qui redort le reliquat tant que la cible n'est pas réellement atteinte. Ne pas revenir à un `Task.Delay(delay)` brut pour ces planifications.
20. **Fichiers statiques non hashés servis sans `Cache-Control` = vieilles traductions en cache** — `nginx.conf` (prod, `Dockerfile.prod`) ne posait aucun header de cache : les bundles JS/CSS (hashés par `angular.json` `outputHashing: "all"`, donc sans risque) et les fichiers non hashés (`i18n/*.json`, `index.html`) recevaient le même traitement implicite. Un navigateur peut alors garder en cache une ancienne version de `i18n/fr.json` après un déploiement — incident du 2026-08-15 : un joueur a vu des clés de traduction brutes à l'écran (`admin.browserId.label` au lieu du texte traduit) juste après un déploiement touchant `fr.json`, alors que le serveur servait déjà la bonne version (vérifié via `curl` — le problème était bien côté cache client, pas serveur). Fix : `Cache-Control: public, max-age=31536000, immutable` sur les JS/CSS hashés, `Cache-Control: no-cache` (revalidation systématique) sur tout le reste (`i18n/`, `index.html`/routes SPA). Vérifié par le job CI `nginx-headers` (`scripts/check-nginx-cache-headers.sh`) — seul job qui construit et sert réellement l'image Docker de prod.
21. **Scanners de sécurité email = pré-visite automatique des liens** — Microsoft Safe Links et certains antivirus d'entreprise suivent automatiquement les liens contenus dans un email avant que le destinataire ne clique lui-même. Un magic link à usage unique consommé automatiquement au chargement de `/login/verify` serait donc grillé avant le vrai clic humain, qui tomberait sur « lien invalide ». Fix : `VerifyLoginComponent` n'appelle **jamais** `VerifyMagicLink` au montage — un bouton « Confirmer la connexion » explicite déclenche l'appel réel, seul un vrai clic humain consomme le token.
22. **`OriginValidator` (anti-CSRF `VerifyMagicLink` + auth admin) casse tout appel non-navigateur si les ports ne sont pas synchronisés** — `LoginEndpoint.IsAdminAuthenticated` vérifie désormais aussi l'`Origin`/`Referer` contre `Cors:AllowedOrigins`. Un client qui n'envoie pas cet en-tête (`HttpClient` des tests d'intégration, `fetch()` Node de la fixture E2E `ApiTestClient`) se fait rejeter en 401/403 même avec un token admin valide. Trois points à tenir synchronisés : `IntegrationTestFactory` pose l'en-tête sur son client partagé, `e2e/fixtures/api-client.ts` le pose sur `ADMIN_HEADERS`, et `appsettings.Testing.json` (`Cors:AllowedOrigins`) doit lister **tous** les ports front réellement utilisés en test (5173 CI, 5174 e2e local, cf. piège 10) — en oublier un casse silencieusement toute la suite E2E qui tourne sur ce port (bug réel introduit puis corrigé lors de l'implémentation des comptes utilisateurs, 2026-08).
23. **`SameSite=None` sur le cookie joueur = déconnexions quotidiennes une fois le domaine devenu same-site** — le cookie `authToken` dure 90 jours côté serveur (`CookieAuthService.CookieLifetime`), mais restait en `SameSite=None; Secure=true` en prod même après le passage de front/back sur `inseconds.cc`/`api.inseconds.cc` (same-site depuis ce moment). Les navigateurs purgent bien plus agressivement les cookies `SameSite=None` que leur durée de vie nominale (Safari ITP en particulier) → symptôme observé : obligation de repasser par le magic link quasi tous les jours, alors que le token du magic link lui-même (15 min, à usage unique) n'était pas en cause. Fix (2026-09-02) : `SameSite=Lax` en prod, suffisant puisque plus aucune requête n'est réellement cross-site vis-à-vis de `api.inseconds.cc` (la redirection 301 `code.run` → `inseconds.cc`, cf. plus bas, garantit qu'aucun client actif ne reste sur l'ancien domaine cross-site).
24. **Server GC (défaut SDK Web) = OOMKilled sur un conteneur à mémoire contrainte** — le service `api`, hébergé un temps sur un plan très limité (512 MB / 0,2 vCPU partagé), subissait des `OOMKilled` répétés. Les métriques montraient la mémoire monter vite après le boot puis rester en plateau à 480-500 MiB pendant des heures avant chaque crash (pas une fuite qui grimpe sans fin). Cause : `Microsoft.NET.Sdk.Web` active **Server GC** par défaut, qui dimensionne son tas par cœur logique et retient la mémoire plutôt que de la rendre à l'OS — inadapté à un conteneur aussi contraint. Fix (2026-09-08) : `InSeconds.Api.csproj` force **Workstation GC** (`ServerGarbageCollection=false`, `ConcurrentGarbageCollection=false`), empreinte mémoire nettement plus faible, débit légèrement inférieur sans impact vu le trafic du projet. En même temps, `IMemoryCache` (`Program.cs`) a reçu une `SizeLimit` par sécurité — attention, dès qu'une `SizeLimit` est configurée sur un `IMemoryCache`, **chaque** `cache.Set(...)` doit déclarer une `Size` explicite (`MemoryCacheEntryOptions.Size`) sinon exception au runtime (piège vécu sur `CachedDeezerClient`). Le VPS actuel a plus de marge mémoire (4 Go), mais ce réglage reste en place — faible coût, pas de raison de revenir en arrière.

## Déjà implémenté (non exhaustif)

- **Comptes utilisateurs — whitelist admin + login par magic link** (2026-08) — le jeu **reste 100% jouable en guest**, sans aucune fermeture d'accès : la whitelist gate uniquement qui peut **créer/utiliser un compte lié**, pas qui peut jouer. Un admin whitelist des emails (`Features/Admin/AllowedEmails/`, CQRS complet : Add/Remove/GetAllowedEmails) ; l'ajout déclenche automatiquement un email d'invitation (lien valide 7 jours). Un email whitelisté demande un lien de connexion (`POST /api/auth/magic-link/request`, `Features/Auth/RequestMagicLink/` — réponse générique systématique, pas de fuite d'info sur l'existence de l'email) valide 15 minutes, envoyé via **l'API Resend** (`Common/Email/ResendEmailSender`, `HttpClient` typé, plan gratuit — remplace l'ancien envoi SMTP direct/MailKit via un compte Gmail, abandonné pour la délivrabilité et la simplicité de config ; couvert par `ResendEmailSenderTests`, unit — payload envoyé, erreur HTTP non-2xx, propagation de l'annulation). `POST /api/auth/magic-link/verify` (`Features/Auth/VerifyMagicLink/`) délègue la résolution "email vérifié → compte" à `Common/Auth/AccountLinkingService`, partagée et prête pour un futur Google OAuth (même clé de résolution : l'email) : première connexion → conversion du `Player` guest courant (`IsGuest=false`, `Pseudo` choisi, historique/streak conservés) ; reconnexion (autre appareil ou même) → résout le `Player` existant, pose son cookie. **Limitation acceptée et documentée** : si la toute première connexion d'un joueur se fait sur un appareil B alors qu'il a joué en guest sur A, la partie de A reste orpheline (pas de fusion construite). `Common/Auth/MagicLinkTokenService`/`MagicLinkTokenGenerator` : génération/hash (SHA-256, jamais le token brut stocké)/expiration mutualisés entre le login self-service et l'invitation admin. **HTML des deux emails externalisé** (`Common/Email/Templates/`, `<EmbeddedResource>` → compilés dans la DLL) : `_layout.html` porte tout le commun (DA du jeu, `<style>`, header, carte, footer) avec des trous `{{SECTION:preheader/body/footer}}`, chaque `magic-link.html` / `whitelist-invitation.html` ne fournit que ses sections (marqueurs `<!--#nom-->`). Assemblé par `Common/Email/EmailTemplateRenderer` (injection des sections + substitution du jeton `{{MAGIC_LINK_URL}}`, pas de moteur de template) ; les sujets restent en `const` C# dans `*EmailTemplate.Build()` (`"Ton lien de connexion IN//SECONDS"` / `"Tu es invité·e à tester les comptes IN//SECONDS"`). `Common/Auth/OriginValidator` : anti-CSRF léger (vérifie `Origin`/`Referer` contre `Cors:AllowedOrigins`) sur `VerifyMagicLink` (cookie posé sur un `POST`) et sur `LoginEndpoint.IsAdminAuthenticated` (défense en profondeur, propagée à tous les endpoints admin). `Pseudo` validé par **allowlist de caractères** (`^[\p{L}\p{N} _.-]{3,20}$`), pas de filtre de mots interdits. Connexion rapide dev (`Features/Auth/DevLogin/`, `POST /api/auth/dev-login`, 3 comptes seed `User1`/`User2`/`User3`) montée **uniquement** sous `IsDevelopment()` — jamais Testing, jamais Production. Front : `PlayerSessionService` (remplace `player-identity.service.ts`, expose `isGuest`/`email`/`pseudo`), écrans `/login` + `/login/verify` (bouton "Confirmer" explicite, cf. piège 21), icône de connexion discrète dans le footer (tooltip pseudo pour un compte lié, `"Se connecter"` pour un guest ; clic sur un compte lié ouvre une pop-up "Compte connecté" avec bouton "Se déconnecter" explicite — plus de déconnexion directe au clic, cf. `features/game/CLAUDE.md`), onglet admin "Emails autorisés", bouton diagnostic "Test email" (`Features/Admin/SendTestEmail/`, remonte l'erreur réelle contrairement au reste), pseudo affiché dans les chips joueur de l'onglet Défis admin (fallback ID tronqué pour les guests).
...
- `SubmitAnswerResponse` inclut : `AverageSecondsWhenCorrect` (moy. temps des joueurs ayant trouvé) + `FailureRatePercent` + `ListenedDurationSeconds` + `GuessTimeDistribution` (`DurationBucketDto[]` : nb de bonnes réponses par palier d'écoute, un bucket par valeur de `AllowedDurationsSeconds`, réponse courante incluse) + `NotFoundCount` (nb de joueurs n'ayant pas trouvé). `TrackStat` (`GET /api/stats/today`) porte les mêmes champs `GuessTimeDistribution` + `NotFoundCount` par morceau (sessions complétées) **plus `Score`** (score du joueur pour ce morceau, `null` sans session complétée — alimente le `+score` de la liste mutualisée `TrackResultsListComponent`). Projection sur les paliers mutualisée dans `Common/Stats/GuessTimeDistribution.Build` ; `DurationBucketDto` vit dans `Common/Stats/`
- **Histogramme « en combien de temps les autres ont trouvé »** (`GuessTimeChartComponent`, `shared/guess-time-chart/`) : affiché sur l'écran de révélation du blind round (à la place de l'ancienne ligne texte « Ton temps / Moy. / Pas trouvé ») **et** en pop-up au clic sur le `+score` d'un morceau — **à la fois sur le récap final et sur l'écran « déjà joué »**, via la liste de morceaux mutualisée `TrackResultsListComponent`. Le récap fournit `stats` = `GET /api/stats/today` (récupéré à l'entrée de l'état `done`), « déjà joué » utilise le même `stats` déjà chargé pour cet écran. **Côté admin** (onglet Défis, `ChallengesTabComponent`) : une icône « graphique » sur chaque carte morceau ouvre la même pop-up **avec les chiffres au-dessus des barres** (`showCounts=true`), alimentée par `TrackStatsDto.GuessTimeDistribution`/`NotFoundCount` (`GET /api/admin/challenge-stats` — endpoint scindé de `/api/admin/stats` le 2026-08-29, chargé seulement à l'ouverture de l'onglet Défis)
...
- `CookieAuthService` — résout ou crée un Player guest, cookie HttpOnly signé (`SameSite=Lax` en prod depuis le passage au domaine public, cf. piège 23 ; `SameSite=None` historiquement sur les URLs `code.run`)
- **Création paresseuse du Player** (2026-08-21) — un simple chargement de page ne crée plus de ligne `Players` en base : `PlayerAuthMiddleware` résout un Player existant depuis le cookie (`ICookieAuthService.TryResolvePlayerAsync`) mais n'en crée jamais. Seuls `POST /api/sessions` (démarrer une partie) et `GET /api/players/me` (admin, `BrowserIdComponent`) appellent `ResolveOrCreatePlayerAsync` pour créer un Player à la demande. Les autres endpoints (`/api/settings`, `/api/deezer/search`, `/api/stats/today`...) fonctionnent sans identité tant que le joueur n'a jamais démarré de partie (`httpContext.GetPlayerIdOrNull()`, nullable). Évite de polluer `Players` avec des visiteurs qui ne jouent jamais. Migration `PurgeUnplayedPlayers` (one-shot, data-only) supprime les Players existants sans aucune `GameSession`.
- **Création paresseuse de la session + peek `GET /api/sessions/today`** — l'écran d'accueil est piloté par un **peek lecture seule** au chargement de la page : `game.component.ngOnInit` appelle `GET /api/sessions/today` (`state` ∈ `no_challenge`/`can_start`/`resumable`/`already_played`/`abandoned`), qui **ne crée ni Player, ni cookie `authToken`, ni `GameSession`**. `POST /api/sessions` n'est déclenché qu'au **clic explicite** « Commencer à jouer » / « Reprendre » / « Abandonner » (depuis l'écran de reprise). Un visiteur qui n'ouvre jamais une partie ne laisse donc **aucune trace en base**. Corollaire côté stats : un `Pending` orphelin = quelqu'un qui a réellement démarré (plus de faux abandons dus au simple chargement de page). Détail endpoint : `src/back/InSeconds.Api/CLAUDE.md` § `Sessions/GetTodaySession`.
...
- Page admin (`/admin`) — login, création de défis, recherche Deezer, reset sessions du jour ; heure de déploiement affichée sous le titre **uniquement quand connecté** (générée au `prebuild` via `scripts/generate-build-info.mjs` → `src/app/core/build-info.ts`, ignoré par git) — s'affiche dans le timezone du navigateur ; stats par défi et historique défis regroupés dans l'onglet Défis, paginés par mois (navigateur ‹ Mois Année › unique en haut de l'onglet, état porté par `AdminStatsService`). Ordre des onglets : Dashboard, Défis, Pool, Actions, Emails autorisés. **Onglet actif persisté dans l'URL** (`?tab=`, 2026-09-02) — un F5 sur `/admin?tab=pool` rouvre directement l'onglet Pool (au lieu de retomber sur Dashboard), synchronisation portée par `AdminStateService.setActiveTab`/`readTabFromUrl` (détail : `features/admin/CLAUDE.md`)
...
- **Déploiement VPS OVH** (front + API + PostgreSQL en Docker derrière Caddy, HTTPS auto), CI/CD auto sur push `main` — détail complet dans [Déploiement VPS](#déploiement-vps) plus haut.
...
- **`ConfirmSheetComponent`** (`shared/confirm-sheet/`) : bottom-sheet de confirmation réutilisable (inputs `title`/`body`/`tone`/`confirmLabel`/`cancelLabel`/`loading`/`confirmStyle`/`cancelStyle`, outputs `confirm`/`cancel`). Mutualise les modales overlay "abandonner" et "quitter" de `game.component`, ainsi que la pop-up "Compte connecté" de `GameFooterComponent` (clic sur l'icône de connexion d'un compte lié → confirmation explicite avant déconnexion, cf. "Comptes utilisateurs" ci-dessous). `body` multi-ligne via `white-space:pre-line` (pas d'`innerHTML`)
...
- **Clés Data Protection persistées en base** — `PersistKeysToDbContext<ApplicationDbContext>()` (table `DataProtectionKeys`, migration `PersistDataProtectionKeys`, package `Microsoft.AspNetCore.DataProtection.EntityFrameworkCore`) : les cookies joueurs survivent aux redémarrages/redéploiements (cf. piège 17).
...
- **Streak joueur** : `Player.CurrentStreak` + `Player.LastPlayedDate` (migration `PlayerStreak`), mis à jour dans `SubmitAnswer/Handler.cs` à la complétion (parties complètes uniquement), basée sur `DailyChallenge.Date` — streak +1 si `LastPlayedDate == date du défi − 1 jour`, `LastPlayedDate` stocke la date du défi (cf. piège 18) — affiché sur l'écran récap final et l'écran "déjà joué"
...
- **Prolongation « écouter plus » libre, sans malus** (2026-07-17) — le joueur peut prolonger l'écoute autant de fois qu'il veut, jusqu'au dernier palier configuré (`AudioPlayerService.extend()`, plus de limite à une seule prolongation). Le score ne dépend que du **palier finalement écouté** (`ScoreCalculator.Calculate` n'a plus de paramètre `wasExtended` ni de malus ×0.75) — décision volontaire : un malus en plus du barème décroissant par palier double-pénalisait un joueur parti prudent sur un petit palier. `AudioPlayerService.extend()` a un comportement dual : si l'audio est **en cours de lecture**, il continue depuis la position réelle jusqu'au nouveau palier (pas de replay) ; si l'audio **n'est pas en lecture** (palier fini ou idle), il relit depuis le début jusqu'au nouveau palier. `GameSessionAnswer.WasExtended` reste stocké (plus aucun effet sur le score) uniquement à des fins de stats admin — voir `ExtendedRate` ci-dessous. Le setting `MaxExtensionsPerAnswer` a été supprimé (jamais appliqué nulle part, cf. historique) — migration `RemoveMaxExtensionsPerAnswerSetting`.
...
- **`GET /api/admin/stats?date=yyyy-MM-dd`** — dashboard admin : KPIs du jour sélectionné (`DailyKpisDto` : complétés, abandons, taux de complétion, score médian), activité 30 jours (tous les jours remplis, jours vides = 0), liste des dates disponibles (`AvailableDates`), répartition joueurs guests/inscrits/actifs. Paramètre `date` optionnel, défaut = aujourd'hui. Jours passés : Pending comptés comme Abandoned dans les KPIs.
- **`GET /api/admin/challenge-stats`** — « Stats par défi » de l'onglet Défis admin, **scindé de `/api/admin/stats` le 2026-08-29** (partie lourde : 30 derniers défis, médiane/moy./min/max score, taux artiste/titre par morceau, `ExtendedRate` = % de réponses avec `WasExtended=true`, histogrammes `GuessTimeDistribution`, `ChallengeStatsDto.Players`). Le front ne l'appelle qu'à l'ouverture de l'onglet Défis (chargement paresseux par onglet, cf. `features/admin/CLAUDE.md`) — l'ouverture de l'admin ne déclenche plus que le `GET /api/admin/stats` léger.
- **Autonomie du pool (onglet Pool admin)** — « X jours de défis restants » affiché en ligne à côté du compteur disponible/utilisé, calculé **côté front** (`AdminPoolService.poolDaysRemaining`), aucune donnée serveur supplémentaire : réutilise `poolTracks` (déjà chargé) et le signal `tracksPerChallenge` du `SettingsService`. `floor(disponibles avec preview ÷ tracksPerChallenge)`, rouge < 3 jours, orange < 7, vert sinon. Mêmes critères que `DailyChallengeGenerator`.
...
- **Tests d'intégration backend** (`InSeconds.Api.IntegrationTests`) : Testcontainers.PostgreSql + `WebApplicationFactory<Program>` + Respawn. ~114 tests couvrant `StartSession`, `GetTodaySession` (`GetTodaySessionTests.cs` — peek : `can_start` sans cookie ni Player créé, `resumable`/`already_played`/`abandoned`, ne crée jamais de session), `SubmitAnswer`, `AbandonSession`, `Stats/Today`, `AdminStats` (Dashboard : KPIs, activité, breakdown) + `ChallengeStats` (`GET /api/admin/challenge-stats`, endpoint scindé — dont `ChallengeStatsDto.Players` statut Completed/Pending/Abandoned/Expired par joueur, `ExpiredCount` vs `AbandonedCount` : Pending d'un jour passé → Expired, bouton → Abandoned ; helper `AdminGetChallengeStatsAsync` dans `AdminTests.cs`), `Players` (`GET /api/players/me`, `PlayersTests.cs` — stabilité de l'ID sur appels répétés, endpoint public sans auth), `PlayerSoftDelete`, `SessionEdgeCases` (expiry paresseuse → `Expired`, streak — dont défi de la veille terminé après minuit UTC et reset après un trou, AbandonSession, UpdateListening), `ChallengeGeneration`, `LazyChallengeGeneration` (régénération à la volée + 503 si pool insuffisant), `Admin/Tracks` (DeleteTrack, UpdateTrack), `Admin/Challenges` (CreateChallenge, GetChallenges), `Admin/RefreshPreviews` (réparation d'un flag corrompu), `Admin/Settings` (`UpdateTrackCooldownTests.cs`), `HealthCheck` (`/health` + date de build + `/health/ready`), `DeezerSearch` (`GET /api/deezer/search` — requête trop courte, requête normale, nettoyage parenthèses + déduplication via le déclencheur `dedup-test` de `FakeDeezerHandler`, pas d'auth requise), **nettoyage des titres affichés** (2026-08-17, 5 tests, un par point d'appel : `SubmitAnswer_TitreAvecParentheses_CorrectTitleEstNettoye`, `StartSession_Reprise_CorrectTitleEstNettoyeDesParentheses`, `TodayStats_TitreAvecParentheses_EstNettoyeDansTrackStat`, `AdminStats_TitreAvecParentheses_EstNettoyeDansTrackStatsDto`, `GetChallenges_TitreAvecParentheses_EstNettoye` — chacun mute `Track.Title` directement en base via `factory.Services.CreateScope()` pour y injecter des parenthèses, aucune donnée de seed n'en contenant nativement). Tournent en mode `Testing` (FakeDeezerHandler + seed auto). Job CI `integration-tests` séparé.
...
- **`ASPNETCORE_ENVIRONMENT=Testing`** : `FakeDeezerHandler` (retourne preview URL locale `test-audio.mp3` pour tous les IDs sauf IDs >= `9_000_000_000` → `preview: ""` pour simuler l'absence de preview), `PurgeSeedData` au démarrage, endpoint `DELETE /api/e2e/reset` + `POST /api/e2e/reseed` (auth admin requise). `/search` : réponse par défaut à un seul morceau, ou déclencheur `q` contenant `dedup-test` → 3 variantes parenthésées du même morceau + un morceau distinct, utilisé par les tests d'intégration/E2E du nettoyage+dédup de l'autocomplete public.
- **Cookie `SameSite=Strict; Secure=false`** en Testing (HTTP local), `SameSite=Lax; Secure=true` en prod (cf. piège 23)
...
- **Health checks** — `GET /health` (liveness) renvoie un JSON `{ status, utc, build }` **consommé par le badge d'état backend du front** (`app.ts`) — ne pas changer les champs existants sans adapter le front (en ajouter est OK). `build` = date UTC de compilation, stampée via `AssemblyMetadata BuildUtc` dans le csproj — permet de vérifier quelle version tourne en prod. `GET /health/ready` (readiness) sonde la DB via `AddDbContextCheck<ApplicationDbContext>` (tag `ready`, renvoie le texte `Healthy`/`Unhealthy`). Endpoints publics (exclus du `PlayerAuthMiddleware` via `StartsWithSegments("/health")`). Package `Microsoft.Extensions.Diagnostics.HealthChecks.EntityFrameworkCore`.
...
- **Tests unitaires frontend** (Karma + Jasmine, job CI `unit-tests-front`) — 179 tests répartis en 19 fichiers : `app.spec.ts`, `game.service.spec.ts`, `settings.service.spec.ts` (dont le fallback `catchError` au boot), `language.service.spec.ts`, `game-footer.component.spec.ts` (toggle langue), `admin-api.service.spec.ts` (incl. `AdminHttpService` et délégation ; `getChallengeStats` distinct de `/challenges`), `admin-state.service.spec.ts` (onglet actif + `visitedTabs`/`hasVisited`/`setActiveTab` sticky, triggers de reload), `admin-stats.service.spec.ts`, `admin-pool.service.spec.ts` (autonomie du pool), `actions-tab.component.spec.ts`, `blind-round.component.spec.ts` (navigation clavier de l'autocomplete — `TestBed.createComponent` + `componentRef.setInput('track', ...)` sans `detectChanges()` pour éviter d'avoir à fournir un `TranslateService` : les méthodes/signals protégés sont exercés directement via bracket-notation, pas le rendu du template ; `DeezerAutocompleteService` stubbée pour renvoyer les suggestions de façon synchrone, sans debounce), `clipboard.service.spec.ts`, `player-identity.service.spec.ts` (appel HTTP déclenché dans le constructeur — `TestBed.inject()` suffit à faire partir la requête), `browser-id.component.spec.ts` (même pattern sans `detectChanges()` que `blind-round`), `challenges-tab.component.spec.ts` (`TestBed.runInInjectionContext(() => new ChallengesTabComponent())` — pas de fixture du tout, `AdminStatsService`/`PlayerIdentityService`/`ClipboardService` stubbés via `useValue`, méthodes `shortId`/`isYou`/`statusBg`/`statusColor`/`copyPlayerId` exercées en bracket-notation). Utilisent `provideHttpClient()` + `provideHttpClientTesting()` + `HttpTestingController` (pas d'ancien `HttpClientTestingModule`).
...
- **Palette CSS centralisée** — variables `:root` dans `styles.scss` (50 propriétés au total depuis la refonte DA "IN//SECONDS", dont 31 sur les préfixes `--bg-*`/`--text-*`/`--border-*`/`--color-*` d'origine ; le reste couvre les nouvelles familles de tokens : `--rgb-*` (canaux RGB bruts pour `rgba()` dynamique), `--gradient-primary`, `--glow-primary`/`--glow-accent-2`/`--glow-success`, `--font-display`/`--font-body`, `--overlay-dark`). Toutes les couleurs des templates passent par `var(--...)`, plus de hex inline. Hovers via classes Tailwind `hover:` (plus de `onmouseenter`/`onmouseleave` JS).
...
- **Optimisations performance back** (2026-07-02) — `.AsNoTracking()` sur toutes les queries lecture-seule (handlers Get/Query/Stats) ; `Select()` projections à la place des `Include().ThenInclude()` dans `StartSession/Handler.cs` (challenge + existingSession) pour ne charger que les colonnes nécessaires ; `Task.WhenAll()` pour paralléliser les queries indépendantes dans `Stats/Today` (`scores`, `trackStats`, `appSettings`) et entre les 5 `Build*` de `GetAdminStats` ; migration EF `AddPerformanceIndexes` : `IX_GameSessions_PlayerStatusChallenge (PlayerId, Status, DailyChallengeId)`, `IX_GameSessionAnswers_DailyChallengeTrackId`, `IX_Players_LastSeenAt` (filtré NOT NULL).
- **Réduction des appels DB** (2026-07-16) — `SubmitAnswer/Handler.cs` fusionne la requête `challengeTrack` avec le check `alreadyAnswered` et calcule les stats de réponse avant le `SaveChangesAsync` (plus d'aller-retour DB après le save, sur le chemin critique de la réponse) ; `StartSession/Handler.cs` centralise le flush des sessions expirées en un seul point au lieu de le dupliquer sur 4 branches de sortie ; `GetTracksHandler` fusionne en une requête (au lieu de deux) via la navigation `Track.DailyChallengeTracks` ; `BuildPlayerBreakdown` dans `GetAdminStats` remplace 4 `CountAsync` par un seul `GroupBy` avec comptages conditionnels ; `CreateChallengeHandler` batch la vérification d'existence des tracks (`WHERE IN` au lieu d'une requête par track) et regroupe les inserts en un seul `SaveChangesAsync` via les propriétés de navigation.
- **Optimisations performance front** (2026-07-02) — `ChangeDetectionStrategy.OnPush` sur tous les composants Angular (26 au 2026-08-17) (les Signals déclenchent automatiquement la détection de changement, `OnPush` est donc sûr et optimal) ; `takeUntilDestroyed(destroyRef)` sur toutes les subscriptions Observables (injected `DestroyRef` dans `game.component.ts`, `blind-round.component.ts`, `admin-pool.service.ts`, `admin-actions.service.ts`) ; stockage des handles `setTimeout` dans `admin-pool.service.ts` (`addToPoolStatusTimer`) et `admin-actions.service.ts` (`generateStatusTimer`) + `clearTimeout()` avant chaque recréation pour éviter l'accumulation de timers.
- **Nettoyage + déduplication de l'autocomplete Deezer joueur** (2026-07-23) — `GET /api/deezer/search` (`SearchEndpoint.CleanAndDeduplicate`, back uniquement) retire les parenthèses/crochets des titres suggérés via `TextNormalizationHelpers.CleanDisplayTitle` (même regex que `TextNormalizer` pour la correction — `TextNormalizationHelpers.ParenthesesPattern()`) puis déduplique sur `(Artist, TitreNettoyé)` en gardant la première occurrence (ordre Deezer = pertinence). Sur-demande `FetchLimit=20` résultats bruts à `DeezerClient`/`CachedDeezerClient` (nouveau paramètre optionnel `limit`, défaut `10`) pour compenser la perte due à la dédup, plafonne le résultat final à 10. La recherche admin (`Admin/Challenges/DeezerSearch`) garde les titres bruts Deezer, nécessaires pour `Track.Title` en BDD. Le joueur qui sélectionne une suggestion soumet donc directement le texte nettoyé (déjà le cas de toute façon côté scoring, `TextNormalizer` retirant les parenthèses à la comparaison). Front inchangé (`DeezerAutocompleteService`/`blind-round` consomment `DeezerSearchResult(Artist, Title)` sans savoir que le nettoyage a eu lieu).
- **Nettoyage des titres affichés au-delà de l'autocomplete** (2026-08-17) — `TextNormalizationHelpers.CleanDisplayTitle` (ex-`SearchEndpoint.CleanTitle`, extrait dans `Common/Text/` pour être réutilisable assembly-wide vu la visibilité `internal`) est désormais appliqué à chaque endroit où un titre de morceau brut (`Track.Title`, potentiellement `"Titre (Radio Edit)"`) est révélé à l'utilisateur : `SubmitAnswer/Handler.cs` (`CorrectTitle` de la réponse — alimente l'écran de résultat par morceau et le récap final), `StartSession/Handler.cs` (`ResumedAnswer.CorrectTitle`, chemin de reprise), `Stats/Today/Handler.cs` (`TrackStat.Title`, écran "déjà joué"), `Admin/Stats/GetChallengeStats/Endpoint.cs` (`TrackStatsDto.Title`, section « Stats par défi » de l'onglet Défis admin — code déplacé depuis `GetAdminStats/` lors du split du 2026-08-29), `Admin/Challenges/GetChallenges/Endpoint.cs` (`TrackDto.Title`, section « Historique » du **même** onglet Défis — ajouté après coup le même jour : un premier passage avait couvert `GetAdminStats` mais oublié `GetChallenges`, ce qui affichait le même morceau nettoyé dans un bloc et brut dans l'autre de la même page). **Ne touche pas** `Track.Title` en base ni la recherche admin (`Admin/Tracks`/`Admin/Challenges/DeezerSearch`), qui doivent rester bruts pour l'ajout/l'actualisation de morceaux et la persistance. Tous ces sites projettent déjà `t.Title` hors requête EF (post-`ToListAsync`/`FirstOrDefaultAsync`, dans le mapping vers le DTO final) donc l'appel à une regex C# ne pose pas de problème de traduction SQL. Aucun changement de forme de DTO (juste le contenu des chaînes), donc pas de régénération NSwag nécessaire.
- **Navigation clavier dans l'autocomplete du blind-round** (2026-07-23) — `BlindRoundComponent.onSearchKeydown` (signal `highlightedIndex`, défaut `-1`) : `↓`/`↑` déplacent la surbrillance dans la dropdown de suggestions avec cycle (wrap-around ; depuis `-1`, `↓` va au premier élément et `↑` va directement au dernier — pattern combobox standard), `Entrée` sélectionne l'élément en surbrillance sans soumettre le formulaire (comme un clic sur la suggestion), sinon laisse le comportement par défaut (soumission normale du round si aucune suggestion n'est en surbrillance). `Échap` ferme la dropdown sans modifier le champ. Highlight visuel via `[style.background]` sur le `<li>` actif (`var(--bg-primary-dk)`), resynchronisé au survol souris (`mouseenter`). `highlightedIndex` réinitialisé à chaque nouvelle réponse de recherche, sélection, effacement (`clearSearch`) et morceau suivant (`next`).
- **Indicateur joueurs par défi + ID navigateur en admin** (2026-08-15) — `GET /api/players/me` (slice `Features/Players/GetCurrentPlayer/`, publique) expose le `PlayerId` déjà résolu par `PlayerAuthMiddleware` pour le cookie du navigateur courant — nécessaire car le cookie `authToken` est `HttpOnly` et ne contient de toute façon pas le `PlayerId` en clair (juste un `AuthToken` chiffré via Data Protection), donc illisible et indéchiffrable côté front. `ChallengeStatsDto.Players` (`{PlayerId, Status, Score}` par session — Completed/Pending/Abandoned) est renvoyé en plus des compteurs agrégés, projeté depuis une seule requête `Sessions` mutualisée (remplaçait 3 requêtes séparées Scores/PendingCount/AbandonedCount) — d'abord dans `GetAdminStats`, aujourd'hui dans `GetChallengeStats` (split du 2026-08-29). Front : `ChallengesTabComponent` affiche un chip ID court par joueur sous chaque défi (clic = copie l'ID complet, surbrillance automatique + libellé « toi » si l'ID correspond au navigateur courant) pour repérer les joueurs qui reviennent d'un jour à l'autre. `BrowserIdComponent` (`shared/browser-id/`) affiche l'ID court du navigateur courant + bouton copier, placé une seule fois dans le header d'`admin.component.html` au-dessus du `@if (authenticated)` — donc visible à la fois sur l'écran de login et dans tout le shell admin. `ClipboardService` (`core/services/`) mutualise la logique `navigator.clipboard.writeText` + gestion d'échec, réutilisé par `game.component.copyToClipboard` (comportement de partage de score inchangé) et les deux nouveaux composants.
- **Headers `Cache-Control` nginx** (2026-08-15) — `nginx.conf` (prod, `Dockerfile.prod`) pose désormais `public, max-age=31536000, immutable` sur les bundles JS/CSS hashés (`outputHashing: "all"`) et `no-cache` (revalidation systématique) sur tout le reste : `i18n/*.json`, `index.html`/routes SPA. Corrige un incident où un navigateur gardait en cache une ancienne version de `i18n/fr.json` après déploiement (clés de traduction affichées en brut, cf. piège 20). Vérifié par le job CI `nginx-headers` (`scripts/check-nginx-cache-headers.sh`) qui construit et sert réellement l'image Docker de prod — seul job à le faire, aucun des trois autres niveaux de test (unitaire/intégration/E2E) ne couvre `nginx.conf`.
...
- **Pas de mot de passe, terrain préparé pour Google OAuth** — le login se fait exclusivement par magic link email (pas de hash/politique de mot de passe à maintenir). La logique "trouver un compte par email, sinon convertir le guest courant" est extraite dans `Common/Auth/AccountLinkingService` (indépendante du moyen de vérification), pour qu'un futur callback Google OAuth n'ait qu'à valider le token Google puis appeler le même service — aucune logique d'identité à dupliquer.
