...
- **Tests unitaires frontend** (Karma + Jasmine, job CI `unit-tests-front`) — 111 tests répartis en 9 fichiers : `app.spec.ts`, `game.service.spec.ts`, `settings.service.spec.ts` (dont le fallback `catchError` au boot), `language.service.spec.ts`, `game-footer.component.spec.ts` (toggle langue), `admin-api.service.spec.ts` (incl. `AdminHttpService` et délégation), `admin-stats.service.spec.ts`, `admin-pool.service.spec.ts` (autonomie du pool), `blind-round.component.spec.ts` (navigation clavier de l'autocomplete — `TestBed.createComponent` + `componentRef.setInput('track', ...)` sans `detectChanges()` pour éviter d'avoir à fournir un `TranslateService` : les méthodes/signals protégés sont exercés directement via bracket-notation, pas le rendu du template ; `DeezerAutocompleteService` stubbée pour renvoyer les suggestions de façon synchrone, sans debounce). Utilisent `provideHttpClient()` + `provideHttpClientTesting()` + `HttpTestingController` (pas d'ancien `HttpClientTestingModule`).
