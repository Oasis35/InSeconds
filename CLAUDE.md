...
21. **`gtag.js` sans signal Google Consent Mode v2 = zéro hit envoyé, silencieusement** — le gtag.js moderne (version unifiée avec le moteur GTM, visible via les événements internes `gtm.dom`/`gtm.load`/`gtm.scrollDepth` dans `dataLayer`) attend un signal explicite `gtag('consent', 'update', {analytics_storage: 'granted'})` avant d'accepter d'envoyer le moindre événement vers `google-analytics.com/g/collect`. Sans ce signal, le script se charge et traite `config` normalement (`dataLayer` correctement rempli, aucune erreur console), mais reste en attente indéfiniment — symptôme identique à un blocage par bloqueur de pub, mais reproductible avec Brave Shields désactivés **et** Edge Tracking Prevention en "Basique" (donc pas un bloqueur). Incident du 2026-08-24, corrigé le jour même : `AnalyticsService.enable()` envoie maintenant `gtag('consent', 'update', {analytics_storage: 'granted'})` avant `'js'`/`'config'` — safe car `enable()` n'est appelée qu'après acceptation explicite de la catégorie "analytics" (`CookieConsentService`), donc pas besoin d'un `consent default` préalable. Ne pas retirer cet appel. `anonymize_ip` (paramètre GA3/Universal Analytics, sans effet en GA4) retiré au passage.
