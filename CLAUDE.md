...
- `SubmitAnswerResponse` inclut : `AverageSecondsWhenCorrect` (moy. temps des joueurs ayant trouvé) + `FailureRatePercent` + `ListenedDurationSeconds` + `GuessTimeDistribution` (`DurationBucketDto[]` : nb de bonnes réponses par palier d'écoute, un bucket par valeur de `AllowedDurationsSeconds`, réponse courante incluse) + `NotFoundCount` (nb de joueurs n'ayant pas trouvé). `TrackStat` (`GET /api/stats/today`) porte les mêmes champs `GuessTimeDistribution` + `NotFoundCount` par morceau (sessions complétées). Projection sur les paliers mutualisée dans `Common/Stats/GuessTimeDistribution.Build` ; `DurationBucketDto` vit dans `Common/Stats/`
- **Histogramme « en combien de temps les autres ont trouvé »** (`GuessTimeChartComponent`, `shared/guess-time-chart/`) : affiché sur l'écran de révélation du blind round (à la place de l'ancienne ligne texte « Ton temps / Moy. / Pas trouvé ») **et** en pop-up sur le récap final au clic sur le score d'un morceau (`FinalRecapScreenComponent`, alimenté par `stats` = `GET /api/stats/today` récupéré à l'entrée de l'état `done`). L'écran « déjà joué » garde son affichage texte
