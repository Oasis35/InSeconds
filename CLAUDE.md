...
- **Tests d'intégration backend** (`InSeconds.Api.IntegrationTests`) : Testcontainers.PostgreSql + `WebApplicationFactory<Program>` + Respawn. 84 tests couvrant `StartSession`, `SubmitAnswer`, `AbandonSession`, `Stats/Today`, `AdminStats`, `PlayerSoftDelete`, `SessionEdgeCases` (expiry paresseuse, streak — dont défi de la veille terminé après minuit UTC et reset après un trou, AbandonSession, UpdateListening), `ChallengeGeneration`, `LazyChallengeGeneration` (régénération à la volée + 503 si pool insuffisant), `Admin/Tracks` (DeleteTrack, UpdateTrack), `Admin/RefreshPreviews` (réparation d'un flag corrompu), `HealthCheck` (`/health` + date de build + `/health/ready`). Tournent en mode `Testing` (FakeDeezerHandler + seed auto). Job CI `integration-tests` séparé.
