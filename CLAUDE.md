...
**SonarCloud** tourne en *Automatic Analysis* (pas de job dans `ci.yml`), configuré via `sonar-project.properties` à la racine. Seule directive à ce jour : `sonar.cpd.exclusions=**/api.generated.ts` — le client NSwag généré produit une paire de méthodes request/process au boilerplate quasi identique par endpoint, que la détection de copier-coller comptait comme « duplication sur le nouveau code » à chaque nouvel endpoint (échec Quality Gate, seuil 3 %). L'exclusion ne porte que sur la CPD ; bugs / code smells / couverture restent analysés partout.
