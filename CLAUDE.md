...
- **Page admin — Pool** : tableau paginé (15 lignes/page), filtres combinables (texte artiste/titre, statut Disponible/Utilisé, preview OK/Manquante), indicateur preview (vert/rouge) via `TrackDto.HasPreview` (appel Deezer en parallèle dans `GetTracksHandler`), sélection multiple + suppression en lot, popup d'ajout avec recherche Deezer + lecteur preview 30s + boutons "Ajouter" / "Ajouter et fermer", bouton "↻ Actualiser" ouvre la modale pré-remplie avec artiste + titre
