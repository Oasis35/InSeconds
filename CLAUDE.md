...
**SonarCloud** tourne en *Automatic Analysis* (pas de job dans `ci.yml`), configuré via `sonar-project.properties` à la racine. Seule directive à ce jour : `sonar.exclusions=**/api.generated.ts` — le client NSwag généré produit une paire de méthodes request/process au boilerplate quasi identique par endpoint, que la détection de copier-coller comptait comme « duplication sur le nouveau code » à chaque nouvel endpoint (échec Quality Gate, seuil 3 %). Exclusion **totale** du fichier car l'Automatic Analysis ignore `sonar.cpd.exclusions` dans ce fichier (ne lit que `sonar.exclusions` / `sonar.test.exclusions` / `sonar.coverage.exclusions`) ; sans conséquence pratique, ce client généré n'étant jamais corrigé à la main.
