...
- `SubmitAnswerResponse` inclut : `AverageSecondsWhenCorrect` (moy. temps des joueurs ayant trouvé) + `FailureRatePercent` + `ListenedDurationSeconds` + `GuessTimeDistribution` (`DurationBucketDto[]` : nb de bonnes réponses par palier d'écoute, un bucket par valeur de `AllowedDurationsSeconds`, réponse courante incluse) + `NotFoundCount` (nb de joueurs n'ayant pas trouvé). L'écran de révélation du blind round affiche `GuessTimeDistribution` + `NotFoundCount` sous forme d'histogramme (`GuessTimeChartComponent`) à la place de l'ancienne ligne texte « Ton temps / Moy. / Pas trouvé » ; le récap final et l'écran « déjà joué » gardent leur affichage texte (via `RoundResult` / `TrackStat`, `AverageSecondsWhenCorrect`/`FailureRatePercent` toujours renvoyés)
