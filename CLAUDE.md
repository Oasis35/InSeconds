...
- **Tests d'intégration backend** (`InSeconds.Api.IntegrationTests`) : Testcontainers.PostgreSql + `WebApplicationFactory<Program>` + Respawn. 98 tests couvrant `StartSession`, `SubmitAnswer`, `AbandonSession`, `Stats/Today`, `AdminStats` (dont `ChallengeStatsDto.Players` — statut Completed/Pending/Abandoned par joueur), `Players` (`GET /api/players/me`, `PlayersTests.cs` — stabilité de l'ID sur appels répétés, endpoint public sans auth), `PlayerSoftDelete`, `SessionEdgeCases` (expiry paresseuse, streak — dont défi de la veille terminé après minuit UTC et reset après un trou, AbandonSession, UpdateListening), `ChallengeGeneration`, `LazyChallengeGeneration` (régénération à la volée + 503 si pool insuffisant), `Admin/Tracks` (DeleteTrack, UpdateTrack), `Admin/RefreshPreviews` (réparation d'un flag corrompu), `HealthCheck` (`/health` + date de build + `/health/ready`), `DeezerSearch` (`GET /api/deezer/search` — requête trop courte, requête normale, nettoyage parenthèses + déduplication via le déclencheur `dedup-test` de `FakeDeezerHandler`, pas d'auth requise), **nettoyage des titres affichés** (2026-08-17, 4 tests, un par point d'appel : `SubmitAnswer_TitreAvecParentheses_CorrectTitleEstNettoye`, `StartSession_Reprise_CorrectTitleEstNettoyeDesParentheses`, `TodayStats_TitreAvecParentheses_EstNettoyeDansTrackStat`, `AdminStats_TitreAvecParentheses_EstNettoyeDansTrackStatsDto` — chacun mute `Track.Title` directement en base via `factory.Services.CreateScope()` pour y injecter des parenthèses, aucune donnée de seed n'en contenant nativement). Tournent en mode `Testing` (FakeDeezerHandler + seed auto). Job CI `integration-tests` séparé.
