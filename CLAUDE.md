...
- **Tests E2E Playwright** : 23 tests jeu + 15 tests admin (38 au total). Jeu : happy path, écran "déjà joué", bouton partage "déjà joué", abandon mid-game, reprise, abandon depuis reprise, sync multi-onglets, pas de défi, partage fin de partie, scoring (palier/mauvaise réponse/partiel), anti-cheat paliers bloqués à la reprise (paliers < durée écoutée masqués), confirmation de sortie (`leave-guard.spec.ts` : annuler reste sur la partie, confirmer navigue vers `/admin`, pas de confirmation hors `playing`), bouton `✕` d'effacement (`clear-search.spec.ts`), overlay "Service indisponible" (`service-down.spec.ts` : `/health` interceptée via `page.route` → overlay visible puis disparaît au retour du back via `page.clock.runFor(5000)`, pas d'overlay à l'état `loading`). Admin : login, pool (tableau, filtres), ajout, suppression, actualisation, actions, liste défis
...
- **Tests unitaires frontend** (Karma + Jasmine, job CI `unit-tests-front`) — 95 tests répartis en 6 fichiers : `app.spec.ts` (1), `game.service.spec.ts` (11), `settings.service.spec.ts` (12), `language.service.spec.ts` (12), `admin-api.service.spec.ts` (20), `admin-stats.service.spec.ts` (29) + `AdminHttpService` (16) et délégation (4) dans `admin-api.service.spec.ts`. Utilisent `provideHttpClient()` + `provideHttpClientTesting()` + `HttpTestingController` (pas d'ancien `HttpClientTestingModule`).
