...
- **Overlay "Service indisponible"** (`ServiceDownComponent`, `features/service-down/`) — `App` (`app.ts`) sonde `GET /health` toutes les 5s (`timer` + `switchMap`, `takeUntilDestroyed`). Si le backend ne répond plus (`health() === 'ko'`), un overlay plein écran bloquant s'affiche par-dessus le `<router-outlet>` (`app.html`) avec retry auto — disparaît seul dès que le back revient (utile pendant un redéploiement). Jamais affiché à l'état `loading` initial (pas de faux positif au boot). `/health` renvoie `{ status, utc }` (cf. health checks).
