...
- **Histogramme « en combien de temps les autres ont trouvé »** (`GuessTimeChartComponent`, `shared/guess-time-chart/`) : affiché sur l'écran de révélation du blind round (à la place de l'ancienne ligne texte « Ton temps / Moy. / Pas trouvé ») **et** en pop-up au clic sur le `+score` d'un morceau — **à la fois sur le récap final et sur l'écran « déjà joué »**, via la liste de morceaux mutualisée `TrackResultsListComponent`. Le récap fournit `stats` = `GET /api/stats/today` (récupéré à l'entrée de l'état `done`), « déjà joué » utilise le même `stats` déjà chargé pour cet écran
