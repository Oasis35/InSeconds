...
21. **`gtag.js` sans `consent default` préalable = zéro hit envoyé, silencieusement (même avec un `consent update`)** — le gtag.js moderne (version unifiée avec le moteur GTM, visible via les événements internes `gtm.dom`/`gtm.load`/`gtm.scrollDepth` dans `dataLayer`) reste en état "consentement non configuré" tant qu'aucun `gtag('consent', 'default', {...})` n'a été déclaré — **un `gtag('consent', 'update', ...)` seul, sans `default` préalable, ne suffit pas** à débloquer l'envoi des hits, contrairement à ce qu'indique la doc Google en apparence. Le script charge, traite `config` normalement (`dataLayer` correctement rempli, aucune erreur console), mais n'envoie jamais de hit `collect` — symptôme identique à un blocage par bloqueur de pub, mais reproductible avec Brave Shields désactivés **et** Edge Tracking Prevention en "Basique" (donc pas un bloqueur). Incident du 2026-08-24 : un premier correctif (ajout d'un `consent update` seul dans `AnalyticsService.enable()`) s'est avéré insuffisant — le vrai diagnostic n'est venu qu'en utilisant **Tag Assistant** (tagassistant.google.com), qui affichait explicitement "Consentement non configuré — l'état de consentement par défaut n'a pas encore été défini" sur l'événement `gtm.init_consent`. Fix définitif : `AnalyticsService.declareDefaultConsent()` (tout refusé par défaut, ne charge aucun script) est appelée **inconditionnellement au tout début du boot** dans `app.config.ts` (avant `LanguageService.init()`, avant même que le bandeau cookies s'affiche), indépendamment du choix de l'utilisateur ; `enable()` (appelée après acceptation) envoie ensuite `gtag('consent', 'update', {analytics_storage: 'granted'})`. Ne jamais retirer l'appel à `declareDefaultConsent()` au boot, même si ça semble redondant avec `enable()`. `anonymize_ip` (paramètre GA3/Universal Analytics, sans effet en GA4) retiré au passage. **Pour diagnostiquer un futur souci GA** : Tag Assistant > onglet Network/Console du navigateur — il montre l'état de consentement réel par événement, ce que le simple filtre `collect` du Network tab ne révèle pas.
...
- **Google Analytics + bandeau cookies** (2026-08, `vanilla-cookieconsent` v3) — `AnalyticsService` (`core/services/analytics.service.ts`) injecte `gtag.js` (ID dans `environment.gaMeasurementId`) uniquement via `enable()`, jamais au boot. `CookieConsentService` (`core/services/cookie-consent.service.ts`) initialise le bandeau dans `app.config.ts` (`provideAppInitializer`, après `LanguageService.init()`) : catégories `necessary` (readOnly) / `analytics`, `onFirstConsent`/`onConsent`/`onChange` appellent `AnalyticsService.enable()`/`disable()` selon `CookieConsent.acceptedCategory('analytics')`. Traductions tirées des fichiers i18n existants (clé `cookieConsent`, chargées via `TranslateService.reloadLang()` pour FR+EN au démarrage — pas de doublon de contenu ailleurs) ; `effect()` dans le constructeur répercute un changement de langue (`LanguageService.current`) via `CookieConsent.setLanguage()`. Thème : variables CSS `--cc-*` du plugin mappées sur la palette `:root` existante dans `styles.scss` (pas de CSS de layout custom), feuille de style du plugin ajoutée dans `angular.json` (`styles: [...]`). Bouton "Gérer les cookies" dans `GameFooterComponent` + sur la page `/privacy` (`privacy.cookie.manage`), tous deux appellent `CookieConsentService.showPreferences()`. **Désactivé en E2E** via le flag `window.__disableCookieConsent` (posé par `e2e/fixtures/test.ts`, même pattern que `__disableHealthPolling`) — le bandeau bloquant interceptrait les clics dans tous les specs ; `environment.e2e.ts` a `gaMeasurementId: ''` (no-op côté `AnalyticsService`).
