...
- **`SessionStatus`** (Pending/Completed/Abandoned/Expired) : `StartSession` retourne `IsResuming=true` + `completedAnswers` si session Pending. Expiry paresseuse : Pending de la veille → **`Expired`** au prochain StartSession (**pas `Abandoned`** — réservé au bouton). Stats/leaderboard filtrés sur `Completed`. Dashboard admin : 3 buckets de non-complétion (`abandonedCount` bouton / `expiredCount` sortie sans terminer / `pendingCount` en cours), convergents entre le KPI du jour et « Stats par défi » (`GetAdminStats` passe `today` aux deux, replie le Pending d'un jour passé sur `expiredCount`)
