...
- **Tests unitaires frontend** (Karma + Jasmine, job CI `unit-tests-front`) — 101 tests répartis en 8 fichiers : `app.spec.ts`, `game.service.spec.ts`, `settings.service.spec.ts` (dont le fallback `catchError` au boot), `language.service.spec.ts`, `game-footer.component.spec.ts` (toggle langue), `admin-api.service.spec.ts` (incl. `AdminHttpService` et délégation), `admin-stats.service.spec.ts`, `admin-pool.service.spec.ts` (autonomie du pool). Utilisent `provideHttpClient()` + `provideHttpClientTesting()` + `HttpTestingController` (pas d'ancien `HttpClientTestingModule`).
