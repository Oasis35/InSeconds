...
- **Nettoyage + déduplication de l'autocomplete Deezer joueur** (2026-07-23) — `GET /api/deezer/search` (`SearchEndpoint.CleanAndDeduplicate`/`CleanTitle`, back uniquement) retire les parenthèses/crochets des titres suggérés (même regex que `TextNormalizer` pour la correction — `TextNormalizationHelpers.ParenthesesPattern()`) puis déduplique sur `(Artist, TitreNettoyé)` en gardant la première occurrence (ordre Deezer = pertinence). Sur-demande `FetchLimit=20` résultats bruts à `DeezerClient`/`CachedDeezerClient` (nouveau paramètre optionnel `limit`, défaut `10`) pour compenser la perte due à la dédup, plafonne le résultat final à 10. **Scope volontairement limité à l'endpoint public** : la recherche admin (`Admin/Challenges/DeezerSearch`) garde les titres bruts Deezer, nécessaires pour `Track.Title` en BDD. Le joueur qui sélectionne une suggestion soumet donc directement le texte nettoyé (déjà le cas de toute façon côté scoring, `TextNormalizer` retirant les parenthèses à la comparaison). Front inchangé (`DeezerAutocompleteService`/`blind-round` consomment `DeezerSearchResult(Artist, Title)` sans savoir que le nettoyage a eu lieu).
