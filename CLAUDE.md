...
**SonarCloud** tourne en *Automatic Analysis* (pas de job dans `ci.yml`). ⚠️ **L'Automatic Analysis de ce projet n'ingère pas `sonar-project.properties`** (testé : sans effet). Les exclusions se posent **dans l'UI SonarCloud** : `Administration → Analysis Scope → Source File Exclusions`. Exclusion en place : `**/api.generated.ts` — le client NSwag généré produit une paire de méthodes request/process au boilerplate quasi identique par endpoint, que la détection de copier-coller comptait comme « duplication sur le nouveau code » à chaque nouvel endpoint (échec Quality Gate, seuil 3 %). Le fichier `sonar-project.properties` racine est conservé uniquement comme documentation d'intention (et serait honoré si on passait un jour à une analyse par scanner en CI).
...
17. **Clés Data Protection : persistées en base — ne pas retirer** (`PersistKeysToDbContext<ApplicationDbContext>()`, table `DataProtectionKeys`, migration `PersistDataProtectionKeys`). Sans ça, les clés vivent dans le filesystem du conteneur et sautent à chaque redéploiement **ou redémarrage** Northflank : le cookie `authToken` (chiffré avec ces clés) devient illisible → nouveau guest créé → streak et historique perdus pour tous les joueurs (bug historique, corrigé 2026-07-14). Note : les cookies émis *avant* le premier déploiement de ce fix seront invalidés une dernière fois (anciennes clés perdues avec le conteneur).
...
- Page admin (`/admin`) — login, création de défis, recherche Deezer, reset sessions du jour ; heure de déploiement affichée sous le titre **uniquement quand connecté** (générée au `prebuild` via `scripts/generate-build-info.mjs` → `src/app/core/build-info.ts`, ignoré par git) — s'affiche dans le timezone du navigateur ; stats par défi et historique défis regroupés dans l'onglet Défis, paginés par mois (navigateur ‹ Mois Année › unique en haut de l'onglet, état porté par `AdminStatsService`). Ordre des onglets : Dashboard, Défis, Pool, Actions
